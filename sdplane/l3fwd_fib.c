...
static inline void
fib_parse_packet (struct rte_mbuf *mbuf, uint32_t *ipv4, uint32_t *ipv4_cnt,
                  uint8_t ipv6[RTE_FIB6_IPV6_ADDR_SIZE], uint32_t *ipv6_cnt,
                  uint8_t *ip_type)
{
...
process_packet (struct rte_mbuf *pkt, uint16_t *hop)
{
  ...
  rfc1812_process (
      (struct rte_ipv4_hdr *) (rte_pktmbuf_mtod (pkt, struct rte_ether_hdr *) +
                               1),
      hop, pkt->packet_type);
#endif
...
static inline void
fib_send_packets (int nb_rx, struct rte_mbuf **pkts_burst, uint16_t portid,
                  struct lcore_conf *qconf)
{
  ...
  if (likely (ipv4_cnt > 0))
    rte_fib_lookup_bulk (qconf->ipv4_lookup_struct, ipv4_arr, hopsv4,
                         ipv4_cnt);

  /* Lookup IPv6 hops if IPv6 packets are present. */
  if (ipv6_cnt > 0)
    rte_fib6_lookup_bulk (qconf->ipv6_lookup_struct, ipv6_arr, hopsv6,
                          ipv6_cnt);
      ...
      for (i = 0; i < nb_deq; i++)
        {
          ...
          hops[i] = nh != FIB_DEFAULT_HOP ? nh : events[i].mbuf->port;
          process_packet (events[i].mbuf, &hops[i]);
          events[i].mbuf->port =
              hops[i] != BAD_PORT ? hops[i] : events[i].mbuf->port;
        }
...
fib_process_event_vector (struct rte_event_vector *vec, uint8_t *type_arr,
                          uint8_t **ipv6_arr, uint64_t *hopsv4,
                          uint64_t *hopsv6, uint32_t *ipv4_arr, uint16_t *hops)
{
  uint32_t ipv4_arr_assem, ipv6_arr_assem;
  ...
  for (i = 0; i < (vec->nb_elem - FIB_PREFETCH_OFFSET); i++)
    {
      rte_prefetch0 (
          rte_pktmbuf_mtod (mbufs[i + FIB_PREFETCH_OFFSET], void *));
  ...
  if (ipv6_cnt > 0)
    rte_fib6_lookup_bulk (lconf->ipv6_lookup_struct,
                          (uint8_t (*)[RTE_FIB6_IPV6_ADDR_SIZE]) ipv6_arr,
                          hopsv6, ipv6_cnt);
  ...
  for (i = 0; i < vec->nb_elem; i++)
    {
      ...
      if (nh != FIB_DEFAULT_HOP)
        ...
        hops[i] = vec->attr_valid ? vec->port : vec->mbufs[i]->port;
    }
      ...
      for (i = 0; i < nb_deq; i++)
        {
          ...
          fib_process_event_vector (events[i].vec, type_arr, ipv6_arr, hopsv4,
                                    hopsv6, ipv4_arr, hops);
        }
  ...
  for (i = 0; i < route_num_v4; i++)
    {
      ...
      ret = rte_fib_add (ipv4_l3fwd_fib_lookup_struct[socketid],
                         route_base_v4[i].ip, route_base_v4[i].depth,
