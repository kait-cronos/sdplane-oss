...
/* l2_switch_tx_flush() flushes the queue'ed packets
...
static inline __attribute__ ((always_inline)) void
l2_switch_send_router_if (struct rte_mbuf *m, unsigned rx_portid,
...
static inline __attribute__ ((always_inline)) void
l2_switch_send_capture_if (struct rte_mbuf *m, unsigned rx_portid,
...
static inline __attribute__ ((always_inline)) void
l2_switch_send_link (struct rte_mbuf *m, unsigned rx_portid,
  ...
  if (eth_type == RTE_ETHER_TYPE_VLAN)
    {
      ...
      if (vswitch_link->tag_id != 0 && vswitch_link->tag_id != vlan_id)
      ...
      else if (vswitch_link->tag_id == 0)
        {
          ...
          rte_vlan_strip (c);
          DEBUG_SDPLANE_LOG (
              L2_SWITCH, "m: %p port[%d]: vlan_id strip: %u -> %u", m,
...
static inline __attribute__ ((always_inline)) void
l2_switch_select (struct rte_mbuf *m, unsigned rx_portid, unsigned rx_queueid)
{
  struct rte_ether_hdr *eth_hdr;
  ...
  DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p received on port: %d queue: %d", m,
  ...
  if (eth_type == RTE_ETHER_TYPE_VLAN)
    {
      ...
      vlan_hdr = (struct rte_vlan_hdr *) (eth_hdr + 1);
      vlan_id = RTE_VLAN_TCI_ID (rte_be_to_cpu_16 (vlan_hdr->vlan_tci));
      DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p tagged: vlan: %u", m, vlan_id);
  ...
  else
    {
      ...
      if (0 <= vswitch_link_id &&
          vswitch_link_id < rib->rib_info->vswitch_link_size)
        {
          ...
          DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p untag: vswitch: %u", m,
  ...
  if (! vswitch_link)
    {
      DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p cannot find the vswitch link", m);
      return;
    }
  ...
  if (rte_is_multicast_ether_addr (&eth_hdr->dst_addr) ||
      rte_is_broadcast_ether_addr (&eth_hdr->dst_addr))
    {
      DEBUG_SDPLANE_LOG (L2_SWITCH,
                         "m: %p multicast/broadcast, flooding to VLAN %u", m,
                         vswitch_link->vswitch_id);
      ...
      for (i = 0; i < vswitch->vswitch_port_size; i++)
        {
          ...
          if (link == vswitch_link || tx_portid == rx_portid)
            {
              DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p split horizon.", m);
              continue;
            }
          ...
          /* forward to dpdk ports, according to the vswitch_link. */
          l2_switch_send_link (m, rx_portid, rx_queueid, tx_portid,
                                    tx_queueid, link);
        }
    }
  else
    {
      dst_port =
          fdb_lookup_entry (rib->rib_info, &eth_hdr->dst_addr, vswitch_link->tag_id);

      if (dst_port >= 0)
        {
          DEBUG_SDPLANE_LOG (L2_SWITCH, "m: %p unicast to known port %d", m,
                             dst_port);
          ...
          for (i = 0; i < vswitch->vswitch_port_size; i++)
            {
              ...
              if (link->port_id == dst_port && link != vswitch_link)
                {
                  l2_switch_send_link (m, rx_portid, rx_queueid, dst_port,
                                            tx_queueid, link);
                  break;
                }
            }
      ...
      else
        {
          ...
          DEBUG_SDPLANE_LOG (L2_SWITCH,
                             "m: %p unknown unicast, flooding to VLAN %u", m,
                             vswitch_link->vswitch_id);
  ...
  if (rif->sockfd >= 0 && rif->ring_up)
    {
      l2_switch_send_router_if (m, rx_portid, rx_queueid, rif);
    }

  if (cif->sockfd >= 0 && cif->ring_up)
    {
      l2_switch_send_capture_if (m, rx_portid, rx_queueid, cif);
    }
      ...
      for (j = 0; j < nb_rx; j++)
        {
          ...
          l2_switch_select (m, portid, queueid);
...
l2_switch (__rte_unused void *dummy)
{
  ...
  DEBUG_SDPLANE_LOG (L2_SWITCH, "entering main loop on lcore %u", lcore_id);
