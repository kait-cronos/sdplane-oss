 ...
 */
static inline void
processx4_step1 (struct rte_mbuf *pkt[FWDSTEP], __vector unsigned int *dip,
                 uint32_t *ipv4_flag)
{
 ...
 */
static inline void
processx4_step2 (const struct lcore_conf *qconf, __vector unsigned int dip,
                 uint32_t ipv4_flag, uint8_t portid,
                 struct rte_mbuf *pkt[FWDSTEP], uint16_t dprt[FWDSTEP])
{
  rte_xmm_t dst;
  const __vector unsigned char bswap_mask =
      (__vector unsigned char){ 3,  2,  1, 0, 7,  6,  5,  4,
                        ...
                        uint8_t portid, struct lcore_conf *qconf)
{
  ...
  l3fwd_lpm_process_packets (nb_rx, pkts_burst, portid, dst_port, qconf, 0);
