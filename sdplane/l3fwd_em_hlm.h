                          ...
                          struct lcore_conf *qconf, const uint8_t do_step3)
{
  ...
  for (j = 0; j < EM_HASH_LOOKUP_COUNT && j < nb_rx; j++)
    {
      rte_prefetch0 (rte_pktmbuf_mtod (pkts_burst[j], struct rte_ether_hdr *) +
                     1);
    }
  ...
  for (j = 0; j < n; j += EM_HASH_LOOKUP_COUNT)
    {
      ...
      for (i = 0, pos = j + EM_HASH_LOOKUP_COUNT;
        ...
        {
          rte_prefetch0 (
              rte_pktmbuf_mtod (pkts_burst[pos], struct rte_ether_hdr *) + 1);
        }
      ...
      if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV4))
      ...
      else
        {
          for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++)
            dst_port[j + i] =
                em_get_dst_port (qconf, pkts_burst[j + i], portid);
        }
  ...
  for (j = 0; j < n; j += EM_HASH_LOOKUP_COUNT)
    {
      ...
      if (tcp_or_udp && (l3_type == RTE_PTYPE_L3_IPV4))
      ...
      else
        {
          for (i = 0; i < EM_HASH_LOOKUP_COUNT; i++)
            {
              pkts_burst[j + i]->port = em_get_dst_port (
                  qconf, pkts_burst[j + i], pkts_burst[j + i]->port);
                               ...
                               struct lcore_conf *qconf, uint16_t *dst_port)
{
  ...
  if (vec->attr_valid)
    l3fwd_em_process_packets (vec->nb_elem, vec->mbufs, dst_port, vec->port,
                              qconf, 1);
  else
    for (i = 0; i < vec->nb_elem; i++)
      l3fwd_em_process_packets (1, &vec->mbufs[i], &dst_port[i],
