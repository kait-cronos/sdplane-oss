...
lpm_get_dst_port_with_ipv4 (const struct lcore_conf *qconf,
                            struct rte_mbuf *pkt, uint32_t dst_ipv4,
                            uint16_t portid)
{
  uint32_t next_hop;
  struct rte_ipv6_hdr *ipv6_hdr;
  ...
  if (RTE_ETH_IS_IPV4_HDR (pkt->packet_type))
  ...
  else if (RTE_ETH_IS_IPV6_HDR (pkt->packet_type))
    {
      ...
      return (
          uint16_t) ((rte_lpm6_lookup (qconf->ipv6_lookup_struct,
...
lpm_process_event_vector (struct rte_event_vector *vec,
                          struct lcore_conf *lconf, uint16_t *dst_port)
{
  ...
  if (vec->attr_valid)
    {
      l3fwd_lpm_process_packets (vec->nb_elem, mbufs, vec->port, dst_port,
                                 lconf, 1);
    }
  else
    {
      for (i = 0; i < vec->nb_elem; i++)
        l3fwd_lpm_process_packets (1, &mbufs[i], mbufs[i]->port, &dst_port[i],
                                   lconf, 1);
    }
#else
  for (i = 0; i < vec->nb_elem; i++)
                       ...
                       const uint8_t flags)
{
  ...
  lcore_id = rte_lcore_id ();
  lconf = &lcore_conf[lcore_id];
  dst_port_list = rte_zmalloc ("", sizeof (uint16_t) * evt_rsrc->vector_size,
                               RTE_CACHE_LINE_SIZE);
      ...
      for (i = 0; i < nb_deq; i++)
        {
          ...
          lpm_process_event_vector (events[i].vec, lconf, dst_port_list);
        }
