                     ...
                     greIp_t *gre)
{
  ...
  gre->ip.version_ihl =
      (IPv4_VERSION << 4) | (sizeof (struct rte_ipv4_hdr) / 4);
...
#undef GRE_DST_ADDR
  ...
  gre->gre.version = 0;
  gre->gre.eth_type = htons (
      RTE_ETHER_TYPE_IPV4); /* FIXME get EtherType of the actual encapsulated
...
char *
pktgen_gre_ether_hdr_ctor (port_info_t *info __rte_unused, pkt_seq_t *pkt,
                           greEther_t *gre)
{
  /* Zero out the header space */
...
#undef GRE_DST_ADDR
  ...
  struct rte_ether_hdr *eth_hdr =
      (struct rte_ether_hdr
           *) ((char *) &gre->gre + 2 /* Flags and version */
               + 2                    /* Protocol type */
  ...
  eth_hdr->ether_type = htons (
      RTE_ETHER_TYPE_IPV4); /* FIXME get Ethernet type from actual
