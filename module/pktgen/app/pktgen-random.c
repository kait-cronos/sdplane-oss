...
pktgen_rnd_bits_init (rnd_bits_t **rnd_bits)
{
  ...
  *rnd_bits = (rnd_bits_t *) rte_zmalloc_socket (
      "Random bitfield structure", sizeof (rnd_bits_t), 0, rte_socket_id ());
 ...
 *
 * DESCRIPTION
 * Set random bit specification. This extracts the 0, 1 and random bitmasks
 * from the provided bitmask template.
 *
...
uint32_t
pktgen_set_random_bitfield (rnd_bits_t *rnd_bits, uint8_t idx, uint8_t offset,
                            const char *mask)
{
...
void
pktgen_rnd_bits_apply (port_info_t *info, struct rte_mbuf **pkts, size_t cnt,
                       rnd_bits_t *rbits)
{
  rnd_bits_t *rnd_bits;
  size_t mbuf_cnt;
          ...
          if (likely (active_specs & 1))
            {
              ...
              pkt_data = (uint32_t *) (&rte_pktmbuf_mtod (
                  pkts[mbuf_cnt], uint8_t *)[bf_spec->offset]);
              ...
              if (bf_spec->rndMask)
                {
                  ...
                  rnd_value =
                      _rnd_func ? _rnd_func () : pktgen_default_rnd_func ();
#else
 ...
 */
void
pktgen_page_random_bitfields (uint32_t print_labels, uint16_t pid,
                              rnd_bits_t *rnd_bits)
{
  ...
  for (bitmask_idx = 0; bitmask_idx < MAX_RND_BITFIELDS; ++bitmask_idx)
    {
      ...
      scrn_printf (
          row++, 1, "%8d %8d %7s   %s", bitmask_idx, curr_spec->offset,
...
pktgen_init_default_rnd (void)
{
  ...
  if (ret != sizeof (xor_state[0]))
    pktgen_log_warning ("Could not read enough random data for PRNG seed (%d)",
                        ret);
 ...
 *
 * RETURNS: Previous random number function (or NULL if the default function
 * was used).
 *
 * SEE ALSO:
 */
