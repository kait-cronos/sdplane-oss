...
#define static_store(p, b)                                                    \
  do                                                                          \
    {                                                                         \
      gtk_tree_store_set (treestore_static[(p)], &toplevel,                   \
                          COL_CHASSIS_PORTS, (b), -1);                        \
    }                                                                         \
  while ((0))
...
#define chassis_store(t, b)                                                   \
  do                                                                          \
    {                                                                         \
      gtk_tree_store_set (treestore_chassis, &(t), COL_CHASSIS_PORTS, (b),    \
                          -1);                                                \
    }                                                                         \
  while ((0))
...
#define stats_store(p, v)                                                     \
  do                                                                          \
    {                                                                         \
      gtk_tree_store_set (treestore_stats[(p)], &toplevel, COL_CHASSIS_PORTS, \
                          (v), -1);                                           \
    }                                                                         \
  while ((0))
...
#define stats_tot_store(p, v)                                                 \
  do                                                                          \
    {                                                                         \
      gtk_tree_store_set (treestore_stats[(p)], &totToplevel,                 \
                          COL_CHASSIS_PORTS, (v), -1);                        \
    }                                                                         \
  while ((0))
...
void
start_taffic_callback (GtkWidget __attribute__ ((unused)) * w,
                       gpointer __attribute__ ((unused)) data)
{
  ...
  gtk_tree_selection_selected_foreach (selection, start_stop_traffic,
                                       (gpointer) &flag);
}
...
void
stop_traffic_callback (GtkWidget __attribute__ ((unused)) * w,
                       gpointer __attribute__ ((unused)) data)
{
  guint flag = 2;
  GtkTreeSelection *selection =
...
void
start_capture_callback (GtkWidget __attribute__ ((unused)) * w,
                        gpointer __attribute__ ((unused)) data)
{
  guint flag = 1;
  GtkTreeSelection *selection =
  ...
  gtk_tree_selection_selected_foreach (selection, start_stop_capture,
                                       (gpointer) &flag);
}
...
void
stop_capture_callback (GtkWidget __attribute__ ((unused)) * w,
                       gpointer __attribute__ ((unused)) data)
{
  guint flag = 2;
  GtkTreeSelection *selection =
...
chassis_tree_view (void)
{
  ...
  g_signal_connect (chassis_view, "row-activated", (GCallback) show_stream,
                    NULL);
...
port_tree_view (unsigned int port_id, const char *title, gboolean is_static)
{
  ...
  if (is_static == TRUE)
    {
      ...
      gtk_tree_view_set_headers_visible (GTK_TREE_VIEW (view_static[port_id]),
                                         TRUE);
    }
  else
    {
      view_stats[port_id] = gtk_tree_view_new ();
      gtk_tree_view_set_headers_visible (GTK_TREE_VIEW (view_stats[port_id]),
                                         TRUE);
    }
  ...
  /* connect 'text' property of the cell renderer to
   *  model column that contains the first name */
  gtk_tree_view_column_add_attribute (col_stats, renderer_stats, "text",
                                      COL_CHASSIS_PORTS);
...
static GtkWidget *
button_box (GtkWidget __attribute__ ((unused)) * view, const char *title,
            gint layout)
{
  GtkWidget *frame;
  GtkWidget *bbox;
  ...
  gtk_widget_set_tooltip_text (GTK_WIDGET (traffic_start_button),
                               "Start Traffic");
  ...
  gtk_widget_set_tooltip_text (GTK_WIDGET (traffic_stop_button),
                               "Stop Traffic");
  ...
  gtk_widget_set_tooltip_text (GTK_WIDGET (capture_start_button),
                               "Start Capture");
 ...
 *
 * DESCRIPTION
 * Accepts pktgen commands from GTK entry, executes it and adds it to multi
 * text
 *
 * RETURNS: N/A
 *
...
console_box (const char *title)
{
  ...
  gtk_table_attach (GTK_TABLE (console), textEntry, 0, 1, 1, 2, GTK_FILL,
                    GTK_FILL, 0, 0);

  gtk_text_buffer_get_iter_at_offset (buffer, &buffer_iter, 0);
  gtk_entry_set_max_length (GTK_ENTRY (textEntry), 250);
  g_signal_connect (textEntry, "activate", G_CALLBACK (console_callback),
                    scrolledwindow);
    ...
    for (i = 0; i < PKTGEN_GUI_MAX_STATIC; i++)
      {
        ...
        gtk_tree_store_set (stats_label_treestore, &toplevel,
                            COL_CHASSIS_PORTS, pktgen_static_fields[i], -1);
      }
    ...
    for (i = 0; i < PKTGEN_GUI_MAX_STATS; i++)
      {
        ...
        gtk_tree_store_append (stats_label_treestore, &toplevel, NULL);
        gtk_tree_store_set (stats_label_treestore, &toplevel,
                            COL_CHASSIS_PORTS, pktgen_stats_fields[i], -1);
      }
...
create_stats_treeview (gboolean is_static)
{
  ...
  gtk_tree_view_append_column (GTK_TREE_VIEW (stats_label_view),
                               stats_label_col);

  stats_label_renderer = gtk_cell_renderer_text_new ();

  gtk_tree_view_column_pack_start (stats_label_col, stats_label_renderer,
                                   TRUE);

  gtk_tree_view_column_add_attribute (stats_label_col, stats_label_renderer,
                                      "text", COL_CHASSIS_PORTS);

  /* set 'weight' property of the cell renderer to
   *  bold print */
  g_object_set (stats_label_renderer, "weight", PANGO_WEIGHT_BOLD,
                "weight-set", TRUE, NULL);

  /* set 'cell-background' property of the cell renderer to light-grey */
  g_object_set (stats_label_renderer, "cell-background", "#D3D3D3",
...
show_static_conf (void)
{
  ...
  gtk_table_attach_defaults (GTK_TABLE (table), create_stats_treeview (TRUE),
                             0, 1, 0, 1);
  ...
  for (pid = 0; pid < RTE_MAX_ETHPORTS; pid++)
    {
      ...
      gtk_table_attach_defaults (GTK_TABLE (table),
                                 port_tree_view (pid, "Port", TRUE), (pid + 1),
                                 (pid + 2), 0, 1);
...
show_statistics (void)
{
  ...
  gtk_box_pack_start (GTK_BOX (hbox_stats), scrolled_window, TRUE, TRUE, 0);
  gtk_box_pack_end (GTK_BOX (hbox_stats), scrolled_tot_window, FALSE, FALSE,
                    0);
  gtk_container_add (GTK_CONTAINER (frame_horz_stats), hbox_stats);
  gtk_scrolled_window_add_with_viewport (
      GTK_SCROLLED_WINDOW (scrolled_tot_window), hbox_tot_stats);
  ...
  /* Create a column of statistics label on the first column of table */
  gtk_table_attach_defaults (GTK_TABLE (table), create_stats_treeview (FALSE),
                             0, 1, 0, 1);
...
start_gui (void)
{
  ...
  gtk_box_pack_start (
      GTK_BOX (hbox),
      button_box (chassis_view, "Traffic", GTK_BUTTONBOX_CENTER), FALSE, FALSE,
      5);
