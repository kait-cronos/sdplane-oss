...
pktgen_print_pcap (uint16_t pid)
{
  ...
  if (pcap == NULL)
    {
      scrn_cprintf (10, this_scrn->ncols,
  ...
  scrn_printf (row++, col, "Port: %d, PCAP Count: %d of %d", pid,
               pcap->pkt_idx, pcap->pkt_count);
  scrn_printf (row++, col, "%*s %*s%*s%*s%*s%*s%*s%*s", 5, "Seq",
               COLUMN_WIDTH_0, "Dst MAC", COLUMN_WIDTH_0, "Src MAC",
               COLUMN_WIDTH_0, "Dst IP", COLUMN_WIDTH_0 + 2, "Src IP", 12,
               "Port S/D", 15, "Protocol:VLAN", 9, "Size-FCS");
  ...
  for (i = pcap->pkt_idx; i < max_pkts; i++)
    {
      ...
      if (type == RTE_ETHER_TYPE_VLAN)
        {
          ...
          proto = ((struct rte_ipv4_hdr *) ((char *) &hdr->u.ipv4 + 4))
                      ->next_proto_id;
        }
      ...
      if (type == RTE_ETHER_TYPE_IPV4)
        {
          scrn_printf (row, col, "%*s", COLUMN_WIDTH_1,
                       inet_ntop4 (buff, sizeof (buff), hdr->u.ipv4.dst_addr,
                                   0xFFFFFFFF));
          col += COLUMN_WIDTH_1;
          scrn_printf (row, col, "%*s", COLUMN_WIDTH_1 + 2,
                       inet_ntop4 (buff, sizeof (buff), hdr->u.ipv4.src_addr,
                                   0xFFFFFFFF));
 ...
 */
static void
pktgen_pcap_mbuf_ctor (struct rte_mempool *mp, void *opaque_arg, void *_m,
                       unsigned i)
{
...
pktgen_pcap_parse (pcap_info_t *pcap, port_info_t *info, unsigned qid)
{
  ...
  /* If count is greater then zero then we allocate and create the PCAP mbuf
   * pool. */
  if (elt_count > 0)
    {
      ...
      if (elt_count < MAX_MBUFS_PER_PORT (pktgen.nb_rxd, pktgen.nb_txd))
        elt_count =
            (MAX_MBUFS_PER_PORT (pktgen.nb_rxd, pktgen.nb_txd) / elt_count) *
            elt_count;

      /* Compute final size of each mbuf by adding the structure header and
       * headroom. */
      ...
      info->q[qid].pcap_mp = rte_mempool_create (
          name, elt_count, max_pkt_size, 0,
          sizeof (struct rte_pktmbuf_pool_private), rte_pktmbuf_pool_init,
          NULL, pktgen_pcap_mbuf_ctor, (void *) pcap,
          rte_eth_dev_socket_id (info->pid), MEMPOOL_F_DMA);
      scrn_printf (0, 0, "\r");
      if (info->q[qid].pcap_mp == NULL)
        pktgen_log_panic ("Cannot init port %d for %d PCAP packets", info->pid,
                          pcap->pkt_count);
      ...
      scrn_printf (
          ...
          16, name, elt_count, pcap->pkt_count, (data_size + 1023) / 1024);
