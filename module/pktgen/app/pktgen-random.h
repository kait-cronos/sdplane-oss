...
#define MAX_BITFIELD_SIZE (sizeof (BITFIELD_T) << 3)
  ...
  void pktgen_rnd_bits_apply (struct port_info_s *info, struct rte_mbuf **pkt,
                              size_t cnt, struct rnd_bits_s *rbits);

  /* Display page with random bitfield settings */
  void pktgen_page_random_bitfields (uint32_t print_labels, uint16_t pid,
