...
GtkWidget *fill_stream_info (proto_type type, unsigned int pid,
                             unsigned int seq_id);
void add_proto_values_column (GtkTreeView *treeview,
...
void start_stop_traffic (GtkTreeModel *model, GtkTreePath *path,
                         GtkTreeIter *iter, gpointer userdata);

void start_stop_capture (GtkTreeModel *model, GtkTreePath *path,
                         GtkTreeIter *iter, gpointer userdata);

GtkWidget *port_tree_view (unsigned int port_id, const char *title,
                           gboolean is_static);

void display_stream_editor (GtkWidget *stream_window, unsigned int pid,
                            unsigned int seq_id);

void stream_apply (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                   gpointer userdata);

void traffic_stream_get_pid (GtkTreeModel *model, GtkTreePath *path,
                             GtkTreeIter *iter, gpointer data);
void traffic_stream_get_seq_id (GtkTreeModel *model, GtkTreePath *path,
                                GtkTreeIter *iter, gpointer data);
...
void enable_stream_callback (GtkCellRendererToggle *cell, gchar *path,
                             gpointer data);

void cell_edited_callback (GtkCellRendererText *cell, const gchar *path_string,
                           gchar *new_text, gpointer data);

void edit_stream_callback (GtkTreeModel *model, GtkTreePath *path,
                           GtkTreeIter *iter, gpointer userdata);

void show_stream_callback (GtkTreeModel *model, GtkTreePath *path,
                           GtkTreeIter *iter, gpointer userdata);
