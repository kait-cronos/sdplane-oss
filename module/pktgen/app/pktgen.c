...
pktgen_packet_rate (port_info_t *info)
{
  ...
  info->tx_pps = pps;
  info->tx_cycles =
      (pktgen.hz * info->tx_burst * get_port_txcnt (pktgen.l2p, info->pid)) /
      ((pps > 0) ? pps : 1);
}
...
pktgen_find_matching_ipsrc (port_info_t *info, uint32_t addr)
{
  ...
  if (ret == -1 ||
      (addr & mask) ==
          (info->seq_pkt[SINGLE_PKT].ip_dst_addr.addr.ipv4.s_addr * mask))
    ret = SINGLE_PKT;
...
pktgen_find_matching_ipdst (port_info_t *info, uint32_t addr)
{
  ...
  /* Now try to match the single packet address */
  if (ret == -1 &&
      addr == info->seq_pkt[SINGLE_PKT].ip_dst_addr.addr.ipv4.s_addr)
    ret = SINGLE_PKT;

  /* Now try to match the range packet address */
  if (ret == -1 &&
      addr == info->seq_pkt[RANGE_PKT].ip_dst_addr.addr.ipv4.s_addr)
    ret = RANGE_PKT;
  ...
  if (mbuf)
    {
      ...
      rte_memcpy (rte_pktmbuf_mtod (mbuf, uint8_t *), (uint8_t *) &pkt->hdr,
                  pktsize);
...
{
  for (int i = 0; i < cnt; i++)
    if (write (info->tx_tapfd, rte_pktmbuf_mtod (mbufs[i], char *),
               mbufs[i]->pkt_len) < 0)
      {
...
static __inline__ void
pktgen_tstamp_check (port_info_t *info, struct rte_mbuf **pkts,
                     uint16_t nb_pkts)
{
  ...
  for (i = 0; i < nb_pkts; i++)
    {
      ...
      if (pktgen_tst_port_flags (info,
                                 ENABLE_LATENCY_PKTS | SEND_RATE_PACKETS))
        {
          tstamp_t *tstamp =
              pktgen_tstamp_pointer (info, rte_pktmbuf_mtod (pkts[i], char *));
          ...
          if (pktgen_tst_port_flags (info, SAMPLING_LATENCIES))
            {
              /* Record latency if it's time for sampling (seperately per
               * lcore) */
              ...
              if (stats->next == 0 || now >= stats->next)
                {
                  ...
                  if (info->latsamp_type == LATSAMPLER_POISSON)
                    {
                      double next_possion_time_ns =
                          next_poisson_time (info->latsamp_rate);
 ...
 *
 * DESCRIPTION
 * If lcore has work to do then return zero else spin till stopped and
 * return 1.
 *
 * RETURNS: 0 or 1
 *
...
pktgen_packet_ctor (port_info_t *info, int32_t seq_idx, int32_t type)
{
  ...
  char *l3_hdr =
      (char *) &eth[1]; /* Point to l3 hdr location for GRE header */
  ...
  if (seq_idx == LATENCY_PKT)
    {
      ...
      if (lat->latency_entropy)
        sport_entropy =
            (uint16_t) (pkt->sport + (tstamp->index % lat->latency_entropy));
    }
  ...
  if (likely (pkt->ethType == RTE_ETHER_TYPE_IPV4))
    {
      if (likely (pkt->ipProto == PG_IPPROTO_TCP))
      ...
      else if (pkt->ipProto == PG_IPPROTO_ICMP)
        {
          ...
          tlen = pkt->pktSize -
                 (pkt->ether_hdr_size + sizeof (struct rte_ipv4_hdr));
  ...
  else if (pkt->ethType == RTE_ETHER_TYPE_ARP)
    {
      ...
      rte_ether_addr_copy (&pkt->eth_src_addr,
                           (struct rte_ether_addr *) &arp->arp_data.arp_sha);
      *((uint32_t *) &arp->arp_data.arp_sha) =
          htonl (pkt->ip_src_addr.addr.ipv4.s_addr);

      rte_ether_addr_copy (&pkt->eth_dst_addr,
                           (struct rte_ether_addr *) &arp->arp_data.arp_tha);
      *((uint32_t *) ((void *) &arp->arp_data +
                      offsetof (struct rte_arp_ipv4, arp_tip))) =
 ...
 *
 * DESCRIPTION
 * Examine a packet and determine its type along with counting statistics
 * around the packet.
 *
 * RETURNS: N/A
 *
  ...
  if (unlikely (flags & (PROCESS_INPUT_PKTS | PROCESS_RX_TAP_PKTS)))
    {
      if (unlikely (flags & PROCESS_RX_TAP_PKTS))
        if (write (info->rx_tapfd, rte_pktmbuf_mtod (m, char *), m->pkt_len) <
            0)
...
static __inline__ void
pktgen_setup_cb (struct rte_mempool *mp, void *opaque, void *obj,
                 unsigned obj_idx __rte_unused)
{
  ...
  rte_memcpy ((uint8_t *) m->buf_addr + m->data_off, (uint8_t *) &pkt->hdr,
              MAX_PKT_SIZE);
  ...
  if (qid == 0)
    {
      uint32_t tstamp = pktgen_tst_port_flags (
          info, (ENABLE_LATENCY_PKTS | SEND_RATE_PACKETS));
 ...
 *
 * DESCRIPTION
 * Handle the main receive set of packets on a given port plus handle all of
 * the input processing if required.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
port_map_info (const char *msg, uint8_t lid, port_mapinfo_t *pm)
{
  ...
  snprintf (buf, sizeof (buf), "  %s processing lcore %3d: rx: %2d tx: %2d",
            msg, pm->lid, pm->rx.cnt, pm->tx.cnt);

  for (int i = 0; i < pm->rx.cnt; i++)
    {
      pm->rx.pids[i] = get_rx_pid (pktgen.l2p, pm->lid, i);

      if ((pm->rx.infos[i] = get_port_private (pktgen.l2p, pm->rx.pids[i])) ==
          NULL)
  ...
  for (int i = 0; i < pm->tx.cnt; i++)
    {
      ...
      if ((pm->tx.infos[i] = get_port_private (pktgen.l2p, pm->tx.pids[i])) ==
          NULL)
        pktgen_log_panic ("Config error: No port %d found on lcore %d\n",
                          pm->tx.pids[i], pm->lid);
  ...
  for (int i = 0; i < pmap.rx.cnt; i++)
    {
      if (pmap.rx.infos[i] == NULL)
        pktgen_log_panic (
            "Invalid RX config: port at index %d not found for %d lcore\n", i,
            lid);
    }
  ...
  for (int i = 0; i < pmap.tx.cnt; i++)
    {
      if (pmap.tx.infos[i] == NULL)
        pktgen_log_panic (
            "Invalid TX config: port at index %d not found for %d lcore\n", i,
            lid);
    }
  ...
  for (int i = 0; i < pmap.rx.cnt; i++)
    {
      ...
      if (dev_sock != SOCKET_ID_ANY && dev_sock != (int) rte_socket_id ())
        pktgen_log_panic (
            ...
            pid, rte_eth_dev_socket_id (pid), rte_lcore_id (),
            rte_socket_id ());
    }
  while (pg_lcore_is_running (pktgen.l2p, lid))
    {
      ...
      if (curr_tsc >= pktgen.tx_next_cycle)
        {
          pktgen.tx_next_cycle =
              pktgen.tx_next_cycle + pmap.tx.infos[0]->tx_cycles;
      ...
      else if (curr_tsc >= pktgen.tx_bond_cycle)
        {
          ...
          for (int i = 0; i < pmap.tx.cnt; i++)
            { /* Transmit zero pkts for Bonding PMD */
...
/**
 *
 * pktgen_main_tx_loop - Main transmit loop for a core, no receive packet
 * handling
 *
 * DESCRIPTION
 * When Tx and Rx are split across two cores this routing handles the tx
 * packets.
 *
 * RETURNS: N/A
 *
  ...
  for (int i = 0; i < pmap.tx.cnt; i++)
    {
      pktgen_log_info ("  Using port/qid %d/%d for Tx on lcore id %d\n",
                       pmap.tx.infos[i]->pid, pmap.tx.qids[i], lid);
      if (pmap.tx.infos[i] == NULL)
        pktgen_log_panic (
            "Invalid TX config: port at index %d not found for %d lcore\n", i,
            lid);
  ...
  while (pg_lcore_is_running (pktgen.l2p, lid))
    {
      ...
      if (curr_tsc >= pktgen.tx_next_cycle)
      ...
      else if (curr_tsc >= pktgen.tx_bond_cycle)
        {
          pktgen.tx_bond_cycle = curr_tsc + pktgen_get_timer_hz () / 10;
          for (int i = 0; i < pmap.tx.cnt; i++)
            { /* Transmit pkts for Bonding PMD */
              if (pktgen_tst_port_flags (pmap.tx.infos[i], BONDING_TX_PACKETS))
                rte_eth_tx_burst (pmap.tx.infos[i]->pid, pmap.tx.qids[i], NULL,
                                  0);
            }
        }
    }
...
pktgen_main_rx_loop (uint8_t lid)
{
  ...
  for (int i = 0; i < pmap.rx.cnt; i++)
    {
      pktgen_log_info ("  Using port/qid %d/%d for Rx on lcore id %d\n",
                       pmap.rx.infos[i]->pid, pmap.rx.qids[i], lid);
      if (pmap.rx.infos[i] == NULL)
        pktgen_log_panic (
            "Invalid RX config: port at index %d not found for %d lcore\n", i,
            lid);
  ...
  while (pg_lcore_is_running (pktgen.l2p, lid))
    for (int i = 0; i < pmap.rx.cnt; i++) /* Read packet */
      pktgen_main_receive (pmap.rx.infos[i], lid, pkts_burst,
                           pmap.rx.infos[i]->rx_burst);
...
pktgen_timer_setup (void)
{
  ...
  rte_timer_reset (&update_stats_timer, hz, PERIODICAL, main_lcore, stats_cb,
                   NULL);
  rte_timer_reset (&update_display_timer, hz / 4, PERIODICAL, main_lcore,
                   display_cb, NULL);
}
