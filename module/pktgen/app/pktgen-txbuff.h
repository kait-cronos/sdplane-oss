...
extern "C"
{
  ...
  typedef void (*tx_buffer_error_fn) (struct rte_mbuf **unsent, uint16_t count,
                                      void *userdata);
 ...
 */
#define TX_BUFFER_SIZE(sz)                                                    \
  (sizeof (struct eth_tx_buffer) + (sz) * sizeof (struct rte_mbuf *))
   ...
   * transmission on the particular port and queue specified. Once the buffer
   * is full of packets, an attempt will be made to transmit all the buffered
   ...
   * 0 if no buffer flush occurred, otherwise the number of packets
   ...
   *   N > 0 = packet has been buffered, and the buffer was subsequently
   * flushed, causing N packets to be sent, and the error callback to be called
   * for the rest.
   */
   ...
   */
  static __rte_always_inline uint16_t
  tx_buffer_bulk (struct eth_tx_buffer *buffer, struct rte_mbuf **tx_pkt,
                  uint16_t nb_pkts)
  {
  ...
  tx_buffer_init (struct eth_tx_buffer *buffer, uint16_t size, uint16_t pid,
                  uint16_t qid)
  {
    int ret = 0;
