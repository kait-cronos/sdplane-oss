      ...
      switch (pkt->ipProto)
        ...
        case PG_IPPROTO_UDP:
          ...
          if (unlikely (range->vlan_id_inc != 0))
            {
              /* Since VLAN is set to MIN_VLAN_ID, check this and skip first
               * increment to maintain the range sequence in sync with other
               * range fields */
              uint32_t p;
      ...
      switch (pkt->ipProto)
        ...
        case PG_IPPROTO_TCP:
          ...
          if (unlikely (! inet6AddrIsUnspecified (range->src_ipv6_inc)))
            {
              ...
              if (memcmp (p, range->src_ipv6_min, sizeof (struct in6_addr)) <
                  0)
          ...
          if (unlikely (! inet6AddrIsUnspecified (range->dst_ipv6_inc)))
            {
              ...
              if (memcmp (p, range->dst_ipv6_min, sizeof (struct in6_addr)) <
                  0)
          ...
          if (unlikely (range->vlan_id_inc != 0))
            {
              /* Since VLAN is set to MIN_VLAN_ID, check this and skip first
               * increment to maintian the range sequence in sync with other
               * range fields */
              uint32_t p;
              static uint8_t min_vlan_set = 0;
              if ((pkt->vlanid == MIN_VLAN_ID) && ! min_vlan_set)
  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      if (info->seq_pkt[RANGE_PKT].ethType == RTE_ETHER_TYPE_IPV6)
      ...
      else
        {
          row++;
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff), htonl (range->dst_ip),
                                   0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->dst_ip_min), 0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->dst_ip_max), 0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->dst_ip_inc), 0xFFFFFFFF));

          row++;
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff), htonl (range->src_ip),
                                   0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->src_ip_min), 0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->src_ip_max), 0xFFFFFFFF));
          scrn_printf (row++, col, "%*s", col_1,
                       inet_ntop4 (buff, sizeof (buff),
                                   htonl (range->src_ip_inc), 0xFFFFFFFF));
        }

      row++;
      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->dst_port,
                range->dst_port_min, range->dst_port_max, range->dst_port_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->src_port,
                range->src_port_min, range->src_port_max, range->src_port_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      info->seq_pkt[RANGE_PKT].ethType == RTE_ETHER_TYPE_IPV6
          ? snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->hop_limits,
                      range->hop_limits_min, range->hop_limits_max,
                      range->hop_limits_inc)
          : snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->ttl,
                      range->ttl_min, range->ttl_max, range->ttl_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->vlan_id,
                range->vlan_id_min, range->vlan_id_max, range->vlan_id_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->cos,
                range->cos_min, range->cos_max, range->cos_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      info->seq_pkt[RANGE_PKT].ethType == RTE_ETHER_TYPE_IPV6
          ? snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d",
                      range->traffic_class, range->traffic_class_min,
                      range->traffic_class_max, range->traffic_class_inc)
          : snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->tos,
                      range->tos_min, range->tos_max, range->tos_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d",
                range->pkt_size + RTE_ETHER_CRC_LEN,
                range->pkt_size_min + RTE_ETHER_CRC_LEN,
                range->pkt_size_max + RTE_ETHER_CRC_LEN, range->pkt_size_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->gtpu_teid,
                range->gtpu_teid_min, range->gtpu_teid_max,
                range->gtpu_teid_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->vxlan_gid,
                range->vxlan_gid_min, range->vxlan_gid_max,
                range->vxlan_gid_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%5d:%5d/%5d/%5d", range->vxlan_vid,
                range->vxlan_vid_min, range->vxlan_vid_max,
                range->vxlan_vid_inc);
      scrn_printf (row++, col, "%*s", col_1, str);
      ...
      snprintf (str, sizeof (str), "%x:%x/%x/%x", range->tcp_seq,
                range->tcp_seq_min, range->tcp_seq_max, range->tcp_seq_inc);
      scrn_printf (row++, col, "%*s", col_1, str);

      snprintf (str, sizeof (str), "%x:%x/%x/%x", range->tcp_ack,
                range->tcp_ack_min, range->tcp_ack_max, range->tcp_ack_inc);
      scrn_printf (row++, col, "%*s", col_1, str);
