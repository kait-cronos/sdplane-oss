...
/**
 *
 * pktgen_packet_capture_init - Initialize memory and data structures for
 * packet capture.
 *
 * DESCRIPTION
  ...
  if (onOff == ENABLE_STATE)
    {
      ...
      if (get_port_rxcnt (pktgen.l2p, info->pid) == 0)
        {
          pktgen_log_warning (
              "Port %d has no RX queue: capture is not possible", info->pid);
          return;
        }

      /* Find an lcore that can capture packets for the requested port */
      uint16_t lid_idx, lid, rxid;
      for (lid_idx = 0; lid_idx < get_port_nb_lids (pktgen.l2p, info->pid);
           ++lid_idx)
        {
    ...
    found_rx_lid:
      if (lid == RTE_MAX_LCORE)
        {
          pktgen_log_warning (
              "Port %d has no rx lcore: capture is not possible", info->pid);
          return;
        }
      ...
      pktgen_log_info (
          ...
          (double) cap->mz->len /
               ...
               * ((double) info->link.link_speed * 1000 * 1000 /
                  8) /* Xbit -> Xbyte */
      ...
      if (cap->nb_pkts > 0)
        {
          ...
          strftime (str_time, sizeof (str_time), "%Y%m%d-%H%M%S",
                    localtime (&t));
          snprintf (filename, sizeof (filename), "pktgen-%s-%d.pcap", str_time,
                    cap->port);
...
void
pktgen_packet_capture_bulk (struct rte_mbuf **pkts, uint32_t nb_dump,
                            capture_t *cap)
{
  ...
  for (i = 0; i < nb_dump; i++)
    {
       ...
       * capturing. The packet data is prepended by the untruncated packet
       * length and the amount of captured data (which can be less than the
       * packet size if DPDK has stored the packet contents in segmented
       * mbufs).
       */
