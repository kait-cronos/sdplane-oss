...
add_entry (GtkTreeStore *tree, GtkTreePath *path)
{
  ...
  if (! check_entry (tree, path))
    {
      if (depth == 1) /* if this is a child of the root node, use NULL instead
                         of iter */
        while (! (gtk_tree_model_iter_n_children (GTK_TREE_MODEL (tree),
                                                  NULL) == (index + 1)))
                        ...
                        gpointer __attribute__ ((unused)) data)
{
  ...
  gtk_tree_store_set (GTK_TREE_STORE (model), &iter, TRAF_STR_SELECT, ! active,
                      -1);
...
void
apply_callback (GtkWidget __attribute__ ((unused)) * w,
                gpointer __attribute__ ((unused)) data)
{
  guint flag = 1;
  ...
  gtk_tree_selection_selected_foreach (selection, stream_apply,
                                       (gpointer) &flag);
}
...
void
stream_apply (GtkTreeModel *model, GtkTreePath __attribute__ ((unused)) * path,
