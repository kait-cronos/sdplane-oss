  ...
  switch (m->index)
    ...
    case 52:
      foreach_port (portlist,
                    range_set_dst_port (info, (char *) (uintptr_t) "start",
                                        atoi (argv[4]));
                    range_set_dst_port (info, (char *) (uintptr_t) "min",
                                        atoi (argv[5]));
                    range_set_dst_port (info, (char *) (uintptr_t) "max",
                                        atoi (argv[6]));
                    range_set_dst_port (info, (char *) (uintptr_t) "inc",
                                        atoi (argv[7])));
      break;
    case 53:
      foreach_port (portlist,
                    range_set_src_port (info, (char *) (uintptr_t) "start",
                                        atoi (argv[4]));
                    range_set_src_port (info, (char *) (uintptr_t) "min",
                                        atoi (argv[5]));
                    range_set_src_port (info, (char *) (uintptr_t) "max",
                                        atoi (argv[6]));
                    range_set_src_port (info, (char *) (uintptr_t) "inc",
                                        atoi (argv[7])));
      break;
    case 55:
    ...
    case 71:
      foreach_port (portlist,
                    range_set_pkt_size (info, (char *) (uintptr_t) "start",
                                        valid_pkt_size (argv[3]));
                    range_set_pkt_size (info, (char *) (uintptr_t) "min",
                                        valid_pkt_size (argv[4]));
                    range_set_pkt_size (info, (char *) (uintptr_t) "max",
                                        valid_pkt_size (argv[5]));
                    range_set_pkt_size (info, (char *) (uintptr_t) "inc",
                                        atoi (argv[6])););
      break;
    case 80:
      foreach_port (portlist,
    ...
    case 100:
      foreach_port (portlist,
                    range_set_gtpu_teid (info, argv[3],
                                         strcmp ("inc", argv[3])
                                             ? valid_gtpu_teid (info, what)
                                             : atoi (what)));
      break;
    case 101:
      foreach_port (portlist,
                    range_set_gtpu_teid (info, (char *) (uintptr_t) "start",
                                         valid_gtpu_teid (info, argv[3]));
                    range_set_gtpu_teid (info, (char *) (uintptr_t) "min",
                                         valid_gtpu_teid (info, argv[4]));
                    range_set_gtpu_teid (info, (char *) (uintptr_t) "max",
                                         valid_gtpu_teid (info, argv[5]));
                    range_set_gtpu_teid (info, (char *) (uintptr_t) "inc",
                                         atoi (argv[6])););
      break;
    case 160:
    ...
    case 171:
      foreach_port (
          portlist, range_set_tos_id (info, (char *) (uintptr_t) "start",
                                      atoi (argv[3]));
    ...
    case 173:
      foreach_port (portlist,
                    range_set_hop_limits (info, (char *) (uintptr_t) "start",
                                          atoi (argv[3]));
                    range_set_hop_limits (info, (char *) (uintptr_t) "min",
                                          atoi (argv[4]));
                    range_set_hop_limits (info, (char *) (uintptr_t) "max",
                                          atoi (argv[5]));
                    range_set_hop_limits (info, (char *) (uintptr_t) "inc",
                                          atoi (argv[6])));
      break;
    case 174:
      foreach_port (portlist,
    ...
    case 175:
      foreach_port (portlist,
                    range_set_traffic_class (
                        info, (char *) (uintptr_t) "start", atoi (argv[3]));
                    range_set_traffic_class (info, (char *) (uintptr_t) "min",
                                             atoi (argv[4]));
                    range_set_traffic_class (info, (char *) (uintptr_t) "max",
                                             atoi (argv[5]));
                    range_set_traffic_class (info, (char *) (uintptr_t) "inc",
                                             atoi (argv[6])));
      break;
 ...
 *
 * SEE ALSO:
 * 	"%|seq|sequence %d %P %m %m %4 %4 %d %d %|ipv4|ipv6 %|udp|tcp|icmp %d
 * %d %d"
 */
...
seq_1_set_cmd (int argc __rte_unused, char **argv)
{
  ...
  foreach_port (portlist, pktgen_set_seq (info, seqnum, &dmac, &smac, &dst,
                                          &src, atoi (argv[7]), atoi (argv[8]),
                                          eth[3], proto[0], atoi (argv[11]),
                                          atoi (argv[12]), teid));
 ...
 *
 * SEE ALSO:
 * 	"%|seq|sequence %d %P dst %m src %m dst %4 src %4 sport %d dport %d
 * %|ipv4|ipv6 %|udp|tcp|icmp vlan %d size %d teid %d"
 */
...
seq_2_set_cmd (int argc __rte_unused, char **argv)
{
  ...
  foreach_port (portlist,
                pktgen_set_seq (info, seqnum, &dmac, &smac, &dst, &src,
                                atoi (argv[12]), atoi (argv[14]), eth[3],
                                proto[0], atoi (argv[18]), atoi (argv[20]),
                                teid));
...
seq_cmd (int argc, char **argv)
{
  ...
  switch (m->index)
    ...
    case 17:
      ...
      break;
    default:
      return cli_cmd_error ("Sequence invalid command", "Sequence", argc,
                            argv);
    }
  return 0;
}
  ...
  switch (m->index)
    ...
    case 10:
      ...
      foreach_port (portlist, _do (switch (n) {
                      ...
                      case 7:
                        ...
                        break;
                      default:
                        return cli_cmd_error ("Set rate command is invalid",
                                              "Rate", argc, argv);
                    }));
      break;
    case 20:
    ...
    case 31:
      ...
      if (p)
        {
          cli_printf (
              "Subnet mask not required, removing subnet mask value\n");
          *p = '\0';
        }
      ip_ver = _atoip (argv[4], PG_IPADDR_V4, &ip, sizeof (ip));
    ...
    case 32:
      ...
      if (! p)
        cli_printf (
            "src IP address should contain /NN subnet value, default /128\n");

      ip_ver =
          _atoip (argv[4], PG_IPADDR_V6 | PG_IPADDR_NETWORK, &ip, sizeof (ip));
      foreach_port (portlist, rate_set_ipaddr (info, 's', &ip, ip_ver));
      break;
    case 33:
      /* Remove the /XX mask value if supplied */
    ...
    case 60: /* payload size */
      foreach_port (portlist,
                    rate_set_value (info, "payload", atoi (argv[4])));
      break;
    case 70: /* overhead */
      foreach_port (portlist,
                    rate_set_value (info, "overhead", atoi (argv[3])));
      break;
    default:
      return cli_cmd_error ("Rate invalid command", "Rate", argc, argv);
    }
