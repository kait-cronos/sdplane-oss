                         ...
                         uint32_t nb_mbufs, int socket_id, int cache_size)
{
  ...
  if (pktgen.verbose)
    pktgen_log_info (
        "    Create: '%-*s' - Memory used (MBUFs %6u x size %6u) = %6lu KB",
        16, name, nb_mbufs, DEFAULT_MBUF_SIZE, sz / 1024);
...
pktgen_config_ports (void)
{
      ...
      if (bus && ! strcmp (rte_bus_name (bus), "pci"))
        {
          ...
          printf ("   %2d: %-12s   %2d     %2d   ", i, dev.driver_name,
                  dev.if_index, rte_dev_numa_node (dev.device));
          vend[0] = device[0] = '\0';
          sscanf (rte_dev_bus_info (dev.device),
                  "vendor_id=%4s, device_id=%4s", vend, device);

          rte_eth_dev_get_name_by_port (i, name);
          snprintf (buff, sizeof (buff), "%s:%s/%s", vend, device,
  ...
  RTE_ETH_FOREACH_DEV (pid)
  {
    ...
    pktgen.port_cnt++;
    pktgen_log_info ("Initialize Port %u -- RxQ %u, TxQ %u", pid, rt.rx,
                     rt.tx);
    ...
    info->seq_pkt =
        rte_zmalloc_socket (buff, (sizeof (pkt_seq_t) * NUM_TOTAL_PKTS),
    ...
    if (pktgen.eth_mtu < info->dev_info.min_mtu)
      {
        pktgen_log_info ("Increasing MTU from %u to %u", pktgen.eth_mtu,
                         info->dev_info.min_mtu);
    ...
    if (pktgen.eth_mtu > info->dev_info.max_mtu)
      {
        pktgen_log_info ("Reducing MTU from %u to %u", pktgen.eth_mtu,
                         info->dev_info.max_mtu);
    ...
    if ((ret = rte_eth_dev_configure (pid, rt.rx, rt.tx, &conf)) < 0)
      pktgen_log_panic (
          "Cannot configure device: port=%d, Num queues %d,%d (%d)%s", pid,
    ...
    if ((ret = rte_eth_dev_set_mtu (pid, pktgen.eth_mtu)) < 0)
      pktgen_log_panic ("Cannot set MTU %u on port %u, (%d)%s", pktgen.eth_mtu,
                        pid, -ret, rte_strerror (-ret));
    ...
    for (int q = 0; q < rt.rx; q++)
      {
        ...
        info->q[q].rx_mp = pktgen_mbuf_pool_create (
            "Default RX", pid, q, info->nb_mbufs, sid, cache_size);
        ...
        if (ret < 0)
          pktgen_log_panic ("rte_eth_rx_queue_setup: err=%d, port=%d, %s", ret,
    ...
    info->special_mp = pktgen_mbuf_pool_create ("Special TX", pid, 0,
    ...
    for (int q = 0; q < rt.tx; q++)
      {
        ...
        if ((err = posix_memalign ((void **) &txbuff, RTE_CACHE_LINE_SIZE,
                                   sz)) < 0)
        ...
        info->q[q].tx_mp = pktgen_mbuf_pool_create (
            "Default TX", pid, q, mbufs_per_port, sid, cache_size);
        ...
        info->q[q].seq_mp = pktgen_mbuf_pool_create (
            "Sequence TX", pid, q, mbufs_per_port, sid, cache_size);
        ...
        if (info->pcaps[q] != NULL)
          {
            if (pktgen_pcap_parse (info->pcaps[q], info, q) == -1)
              {
                pktgen_log_panic ("Cannot load PCAP file for port %d queue %d",
                                  pid, q);
              }
          }
        ...
        if (ret < 0)
          pktgen_log_panic ("rte_eth_tx_queue_setup: err=%d, port=%d, %s", ret,
                            pid, rte_strerror (-ret));
        if (pktgen.verbose)
          pktgen_log_info ("");
      }
    ...
    pktgen_log_info (
        "   Src MAC %02x:%02x:%02x:%02x:%02x:%02x <Promiscuous is %s>",
    ...
    /* Copy the first Src MAC address in SINGLE_PKT to the rest of the sequence
     * packets. */
  ...
  RTE_ETH_FOREACH_DEV (pid)
  {
    ...
    if ((ret = rte_eth_dev_start (pid)) < 0)
      pktgen_log_panic ("rte_eth_dev_start: port=%d, %s", pid,
                        rte_strerror (-ret));
  }
  ...
  for (sid = 0; sid < coremap_cnt (pktgen.core_info, pktgen.core_cnt, 0);
       sid++)
