...
calculate_rate (port_info_t *info)
{
  ...
  rate->bytes_per_vframe =
      (rate->vlines * rate->pixels * rate->color_bits) / 8;
...
update_rate_values (port_info_t *info)
{
  ...
  info->seq_pkt[RATE_PKT].pktSize =
      (rate->payload + rate->overhead) - RTE_ETHER_CRC_LEN;
  ...
  if (! strcmp (what, "fps"))
  ...
  else if (! strcmp (what, "payload"))
    {
      if ((value + rate->payload) <=
          (uint32_t) (pktgen.eth_max_pkt - RTE_ETHER_CRC_LEN))
        rate->payload = value;
    }
  else if (! strcmp (what, "overhead"))
    {
      if ((value + rate->overhead) <=
          (uint32_t) (pktgen.eth_max_pkt - RTE_ETHER_CRC_LEN))
        rate->overhead = value;
    }
}
...
{
  rte_memcpy (&info->seq_pkt[RATE_PKT], &info->seq_pkt[SINGLE_PKT],
              sizeof (pkt_seq_t));
...
rate_print_static_data (void)
{
  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      pktgen_display_set_color ("stats.stat.values");
      snprintf (buff, sizeof (buff), "%d /%5d:%5d",
  ...
  scrn_printf (LINK_STATE_ROW, col, "%*s", COLUMN_WIDTH_3,
               "----TotalRate----");
  scrn_eol ();
  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      snprintf (buff, sizeof (buff), "%" PRIu64 "/%" PRIu64, lat->min_cycles,
                lat->max_cycles);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
      ...
      row++;
      snprintf (buff, sizeof (buff), "%d/%d/%d/%d", rate->fps, rate->vlines,
                rate->pixels, rate->color_bits);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
      ...
      snprintf (buff, sizeof (buff), "%d/%d", rate->pkts_per_vframe,
                rate->total_pkts_per_sec);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
