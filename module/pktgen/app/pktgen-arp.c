...
arp_pkt_dump (struct rte_mbuf *m)
{
  ...
  arp = rte_pktmbuf_mtod_offset (m, struct rte_arp_hdr *,
  ...
  printf (
      "  ARP Header Type: %04x, Proto: %04x, hlen: %d, plen: %d, opcode: %d\n",
      ntohs (arp->arp_hardware), ntohs (arp->arp_protocol), arp->arp_hlen,
      arp->arp_plen, ntohs (arp->arp_opcode));
...
void
pktgen_process_arp (struct rte_mbuf *m, uint32_t pid, uint32_t qid,
                    uint32_t vlan)
{
  port_info_t *info = &pktgen.info[pid];
  pkt_seq_t *pkt = NULL;
  ...
  if (arp->arp_opcode == htons (ARP_REQUEST))
    {
      ...
      if (arp->arp_data.arp_tip == arp->arp_data.arp_sip)
        { /* GARP Packet */
      ...
      if (likely (idx >= 0))
        {
          ...
          /* Grab the source MAC address as the destination address for the
           * port. */
