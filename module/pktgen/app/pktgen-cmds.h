...
"C"
{
  ...
  void single_set_mac (port_info_t * info, const char *which,
  ...
  void single_set_port_value (port_info_t * info, char type,
                              uint32_t portValue);
  void single_set_qinqids (port_info_t * info, uint16_t outerid,
                           uint16_t innerid);
  void single_set_vxlan (port_info_t * info, uint16_t flags, uint16_t group_id,
                         uint32_t vxlan_id);
  ...
  /* Range commands */
  void range_set_dest_mac (port_info_t * info, const char *what,
                           struct rte_ether_addr *mac);
  void range_set_src_mac (port_info_t * info, const char *what,
                          struct rte_ether_addr *mac);
  ...
  void range_set_qinqids (port_info_t * info, uint16_t outerid,
                          uint16_t innerid);
  void range_set_gre_key (port_info_t * info, uint32_t gre_key);
  void range_set_ttl (port_info_t * info, char *what, uint8_t ttl);
  void range_set_hop_limits (port_info_t * info, char *what,
                             uint8_t hop_limits);
  void range_set_traffic_class (port_info_t * info, char *what,
                                uint8_t traffic_class);

  /* Sequence */
  void pktgen_set_port_seqCnt (port_info_t * info, uint32_t cnt);
  void pktgen_set_seq (
      port_info_t * info, uint32_t seqnum, struct rte_ether_addr * daddr,
