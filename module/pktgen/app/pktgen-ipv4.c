 ...
 *
 * DESCRIPTION
 * Process a input packet for IPv4 ICMP echo request and send response if
 * needed.
 *
 * RETURNS: N/A
 *
...
void
pktgen_process_ping4 (struct rte_mbuf *m, uint32_t pid, uint32_t qid,
                      uint32_t vlan)
{
  port_info_t *info = &pktgen.info[pid];
  ...
  if ((rte_atomic32_read (&info->port_flags) & ICMP_ECHO_ENABLE_FLAG) &&
      (ip->next_proto_id == PG_IPPROTO_ICMP))
    {
      struct rte_icmp_hdr *icmp =
          (struct rte_icmp_hdr *) ((uintptr_t) ip +
                                   sizeof (struct rte_ipv4_hdr));
      uint16_t cksum =
          ~rte_raw_cksum (icmp, (m->data_len - sizeof (struct rte_ether_hdr) -
                                 sizeof (struct rte_ipv4_hdr)));
      ...
      if (unlikely (icmp->icmp_type == ICMP4_ECHO))
        {
          ...
          icmp->icmp_cksum = rte_raw_cksum (
              icmp, (m->data_len - sizeof (struct rte_ether_hdr) -
                     sizeof (struct rte_ipv4_hdr)));
