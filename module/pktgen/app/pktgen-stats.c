  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      pktgen_display_set_color ("stats.stat.values");
      snprintf (buff, sizeof (buff), "%d /%3d:%3d",
                pkt->pktSize + RTE_ETHER_CRC_LEN, info->rx_burst,
                info->tx_burst);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
      snprintf (buff, sizeof (buff), "%d/%5d/%5d", pkt->ttl, pkt->sport,
                pkt->dport);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
      ...
      snprintf (buff, sizeof (buff), "%3d/%3d/%3d", pkt->cos, pkt->tos >> 2,
                pkt->tos >> 5);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);

      snprintf (buff, sizeof (buff), "%04x/%5d/%5d", pkt->vni_flags,
 ...
 * routine to try and wait for the link status for 3 seconds. If the <wait>
 * flag is zero the try three times to get a link status if the link is not up.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
pktgen_page_stats (void)
{
  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      snprintf (buff, sizeof (buff), "%d:%s", pid + sp,
                pktgen_flags_string (info));
      ...
      snprintf (buff, sizeof (buff), "%'" PRIu64 "/%'" PRIu64, sizes.runt,
                sizes.jumbo);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
      ...
      snprintf (buff, sizeof (buff), "%'" PRIu64 "/%'" PRIu64, prev->ierrors,
                prev->oerrors);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
  ...
  scrn_eol ();
  pktgen_display_set_color ("stats.port.totals");
  scrn_printf (row++, col, "%'*llu", COLUMN_WIDTH_3,
               pktgen.max_total_ipackets);
  scrn_printf (row++, col, "%'*llu", COLUMN_WIDTH_3,
...
pktgen_process_stats (void)
{
  ...
  RTE_ETH_FOREACH_DEV (pid)
  {
    ...
    memcpy (&info->curr_stats.q_ipackets[0], &queue->q_ipackets[0],
            sizeof (queue->q_ipackets) + sizeof (queue->q_opackets) +
                sizeof (queue->q_ibytes) + sizeof (queue->q_ibytes) +
  ...
  for (q = 0; q < RTE_ETHDEV_QUEUE_STAT_CNTRS; q++)
    {
      ...
      if (! hdr)
        {
          ...
          row++;
          pktgen_display_set_color ("stats.port.status");
          scrn_printf (row++, 1, " Rate/s %15s %15s %17s %17s %15s",
                       "ipackets", "opackets", "ibytes MB", "obytes MB",
                       "errors");
          pktgen_display_set_color ("stats.stat.values");
        }
      ...
      scrn_printf (row++, 1, "  Q %2d: %'15lu %'15lu %'17lu %'17lu %'15lu", q,
                   rxpkts, txpkts, rxbytes, txbytes, errors);
    }
  pktgen_display_set_color (NULL);
...
_xstats_display (uint16_t port_id)
{
  ...
  if (info->names == NULL)
    {
      ...
      if (info->cnt !=
          rte_eth_xstats_get_names (port_id, info->names, info->cnt))
        {
  ...
  rte_memcpy (info->prev, info->xstats,
              sizeof (struct rte_eth_xstat) * info->cnt);
  printf ("\n");
  scrn_eol ();
  printf ("\n");
}
...
pktgen_page_xstats (uint16_t pid)
{
  ...
  scrn_printf (3, 1, "     %-32s| %12s | %-32s| %12s |\n", "XStat Name",
               "Per/Second", "XStat Name", "Per/Second");
  pktgen_display_set_color ("top.page");
  printf ("Port ");
  pktgen_display_set_color ("stats.port.status");
  for (k = 0; k < 97; k++)
