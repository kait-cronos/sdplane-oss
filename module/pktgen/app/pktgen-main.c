...
{
  printf (
      "Usage: %s [EAL options] -- [-h] [-v] [-P] [-G] [-g host:port] [-T] [-f cmd_file] [-l "
      "log_file] [-s "
...
pktgen_parse_args (int argc, char **argv)
{
  ...
  pktgen.verbose = 0;
  while ((opt = getopt_long (argc, argvopt, "p:m:f:l:s:g:hPNGTvjtrc", lgopts,
                             &option_index)) != EOF)
    switch (opt)
      {
      case 't':
      ...
      case 'j':
        ...
        pktgen_log_info ("**** Jumbo Frames of %d enabled.",
                         pktgen.eth_max_pkt);
        break;
      ...
      case 'g': /* Define the port number and IP address used for the socket
                   connection. */
        pktgen.flags |= (ENABLE_GUI_FLAG | IS_SERVER_FLAG);
        ...
        if (p == NULL) /* No : symbol means pktgen is a server application. */
        ...
        else
          {
            ...
            *p = '\0';
            ...
            pktgen_log_info (">>> Socket GUI support %s%c0x%x",
