...
#define pktgen_log_trace(fmt, ...)                                            \
  pktgen_log (LOG_LEVEL_TRACE, __FILE__, __LINE__, __func__, fmt,             \
              ##__VA_ARGS__)
#else
...
#define pktgen_log_debug(fmt, ...)                                            \
  pktgen_log (LOG_LEVEL_DEBUG, __FILE__, __LINE__, __func__, fmt,             \
              ##__VA_ARGS__)
#else
...
#define pktgen_log_warning(fmt, ...)                                          \
  pktgen_log (LOG_LEVEL_WARNING, __FILE__, __LINE__, __func__, fmt,           \
              ##__VA_ARGS__)
#else
...
#define pktgen_log_error(fmt, ...)                                            \
  pktgen_log (LOG_LEVEL_ERROR, __FILE__, __LINE__, __func__, fmt,             \
              ##__VA_ARGS__)
#else
...
#define pktgen_log_panic(fmt, ...)                                            \
  do                                                                          \
    {                                                                         \
      scrn_destroy ();                                                        \
      pktgen_log (LOG_LEVEL_PANIC, __FILE__, __LINE__, __func__, fmt,         \
                  ##__VA_ARGS__);                                             \
      rte_panic (fmt "\n", ##__VA_ARGS__);                                    \
    }                                                                         \
  while (0)
...
#define strncatf(dest, fmt, ...)                                              \
  ...
  /**
   *
   * pktgen_log_set_screen_level - Set level of messages that are printed to
   * the screen
   *
   * DESCRIPTION
   ...
   * SEE ALSO:
   */
  void pktgen_log (int level, const char *file, long line, const char *func,
                   const char *fmt, ...);
