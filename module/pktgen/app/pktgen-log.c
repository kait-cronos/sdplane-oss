...
pktgen_log (int level, const char *file, long line, const char *func,
            const char *fmt, ...)
{
  log_msg_t *curr_msg;
  va_list args;
...
#define MAX_PAGE_LINES 28
  ...
  /* Header line */
  scrn_printf (row++, 1, "%1s %8s %-32s %s", "L", "Time", "Function",
               "Message");
 ...
 *
 * DESCRIPTION
 * Generates a string representation of the log entry, suitable for the log
 * page. No effort is made to prettify multi-line messages: if indentation of
 * multiple lines is required, the log msg itself must contain appropriate
 * whitespace.
 *
...
pktgen_format_msg_file (const log_msg_t *log_msg)
{
  ...
  snprintf (msg, sizeof (msg), "%s %s.%03ld [%s:%ld(%s)] %s",
            ...
            : (log_msg->level == LOG_LEVEL_PANIC)   ? "PP"
                                                    : "??",
            timestamp, log_msg->tv.tv_usec / 1000, basename (file),
