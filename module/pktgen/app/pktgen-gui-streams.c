...
fill_proto_field_info (proto_type type, unsigned int pid, unsigned int seq_id)
{
  ...
  if (type == TYPE_ETH)
  ...
  else if (type == TYPE_IPv4)
    {
      ...
      val.name = g_strdup (pktgen_ipv4_fields[i++]);
      val.value =
          g_strdup ((pkt->ipProto == PG_IPPROTO_UDP) ? "UDP" : "User Defined");
      g_array_append_vals (packet_info, &val, 1);
      ...
      val.value = g_strdup (
          inet_ntop4 (buff, sizeof (buff),
                      ntohl (pkt->ip_src_addr.addr.ipv4.s_addr), 0xFFFFFFFF));
      g_array_append_vals (packet_info, &val, 1);

      val.name = g_strdup (pktgen_ipv4_fields[i++]);
      val.value = g_strdup (
          inet_ntop4 (buff, sizeof (buff),
                      ntohl (pkt->ip_dst_addr.addr.ipv4.s_addr), 0xFFFFFFFF));
      g_array_append_vals (packet_info, &val, 1);
    }
  ...
  for (; i < packet_info->len; i++)
    {
      ...
      gtk_list_store_set (model, &iter, COLUMN_NAME,
                          g_array_index (packet_info, protocol, i).name,
                          COLUMN_VALUE,
                          g_array_index (packet_info, protocol, i).value,
...
void
cell_edited_callback (GtkCellRendererText *cell, const gchar *path_string,
    ...
    case COLUMN_VALUE:
      {
          ...
          if ((atoi (new_text) > 4096) || (atoi (new_text) < 1))
            {
              ...
              dialog = gtk_message_dialog_new (
                  GTK_WINDOW (stream_window), GTK_DIALOG_DESTROY_WITH_PARENT,
        ...
        if ((0 != strcmp (old_text, "<auto>")) &&
            (0 != strcmp (old_text, "IPv4")) &&
            (0 != strcmp (old_text, "UDP")))
          {
            ...
            g_free (g_array_index (packet_info, protocol, (i + index_offset))
                        .value);
            g_array_index (packet_info, protocol, (i + index_offset)).value =
                g_strdup (new_text);

            gtk_list_store_set (
                GTK_LIST_STORE (model), &iter, column,
                g_array_index (packet_info, protocol, (i + index_offset))
                    .value,
                -1);
          }
        else
...
add_proto_values_column (GtkTreeView *treeview, GtkTreeModel *stream_model)
{
  ...
  g_signal_connect (renderer, "edited", G_CALLBACK (cell_edited_callback),
                    stream_model);
  g_object_set_data (G_OBJECT (renderer), "column",
...
fill_stream_info (proto_type type, unsigned int pid, unsigned int seq_id)
{
  ...
  gtk_box_pack_start (GTK_BOX (vbox), gtk_label_new ("Edit Protocol fields"),
                      FALSE, FALSE, 0);
...
void
close_window_callback (GtkWidget __attribute__ ((unused)) * widget,
                       gpointer window)
{
  ...
  if (((size - RTE_ETHER_CRC_LEN) < (unsigned int) MIN_PKT_SIZE) ||
      ((size - RTE_ETHER_CRC_LEN) > (unsigned int) MAX_PKT_SIZE))
    {
      ...
      dialog = gtk_message_dialog_new (
          ...
          (MIN_PKT_SIZE + RTE_ETHER_CRC_LEN),
          (MAX_PKT_SIZE + RTE_ETHER_CRC_LEN));
      gtk_window_set_title (GTK_WINDOW (dialog), "Pktgen");
      gtk_dialog_run (GTK_DIALOG (dialog));
      gtk_widget_destroy (dialog);
      ...
      if (l4_text_view->parent != GTK_WIDGET (udp_sw))
        {
          ...
          pkt->ipProto = PG_IPPROTO_USR_DEF; /* Set this value to bypass ctor
                                                to fill l4 protocols */
        }
    }
  else
    {
...
void
display_stream_editor (GtkWidget *window, unsigned int pid,
                       unsigned int seq_id)
{
  ...
  packet_info =
      g_array_sized_new (FALSE, FALSE, sizeof (protocol), (1 + 3 + 11 + 4));
  ...
  GtkWidget *radio_button_udp =
      gtk_radio_button_new_with_label (NULL, "UDP   ");
  ...
  g_signal_connect (G_OBJECT (radio_button_usrdef), "clicked",
                    G_CALLBACK (radio_options_callback), (gpointer) &pid);
  ...
  gtk_box_pack_start (GTK_BOX (stream_l2_vbox), stream_l2_hbox, FALSE, FALSE,
                      0);
...
set_stream_info (unsigned int pid, unsigned int seq_id)
{
  ...
  for (i = 0; i < packet_info->len; i++)
    {
      ...
      switch (i)
        ...
        case 13: /* Source IP Address */
          inet_aton (g_array_index (packet_info, protocol, i).value,
                     &new_src_ip.sin_addr);
          pkt->ip_src_addr.addr.ipv4.s_addr =
              ntohl (new_src_ip.sin_addr.s_addr);
          break;
        ...
        case 14: /* Destination IP Address */
          ...
          pkt->ip_dst_addr.addr.ipv4.s_addr =
              ntohl (new_dst_ip.sin_addr.s_addr);
          break;
  ...
  for (i = 0; i < strlen (usr_def_str) / 2; i++)
    usr_def[i + sizeof (struct rte_ether_hdr) + sizeof (struct rte_ipv4_hdr)] =
        l4_and_payload[i];
...
edit_stream (void)
{
  ...
  g_signal_connect (stream_window, "destroy", G_CALLBACK (gtk_main_quit),
                    NULL);
...
stream_box (void)
{
  ...
  gtk_signal_connect (GTK_OBJECT (apply_button), "clicked",
                      GTK_SIGNAL_FUNC (apply_callback), NULL);
  ...
  for (pid = 0; pid < RTE_MAX_ETHPORTS; pid++)
    {
      ...
      /* Set seqCnt to maximum as enable/disable stream will take care of
       * sending selected stream from GUI */
      port_info_t *info = &pktgen.info[pid];
      info->seqCnt = NUM_SEQ_PKTS;

      /* create tree model */
      traffic_stream[pid] = gtk_tree_store_new (
          NUM_TRAF_STR_COLS, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_INT);
      ...
      while (port_traffic->path != NULL)
        {
          ...
          gtk_tree_model_get_iter (GTK_TREE_MODEL (traffic_stream[pid]), &iter,
                                   path);
          gtk_tree_path_free (path);
          gtk_tree_store_set (traffic_stream[pid], &iter, TRAF_STR_NAME,
                              port_traffic->stream_name, TRAF_STR_SELECT,
                              port_traffic->stream_select, TRAF_STR_NO,
      ...
      gtk_tree_view_column_add_attribute (sl_no_col, num_renderer, "text",
                                          TRAF_STR_NO);
      gtk_tree_view_column_add_attribute (stream_name_col, name_renderer,
                                          "text", TRAF_STR_NAME);
      gtk_tree_view_column_add_attribute (select_col, check_renderer, "active",
                                          TRAF_STR_SELECT);
      ...
      g_signal_connect (stream_view[pid], "row-activated",
                        (GCallback) edit_stream, NULL);
    }
