...
pktgen_get_portlist (lua_State *L, int index)
{
  ...
  if (lua_isstring (L, index))
    {
      if (portlist_parse (luaL_checkstring (L, 1), pktgen.nb_ports,
                          &portlist) < 0)
...
pktgen_set (lua_State *L)
{
  ...
  foreach_port (
      ...
      _do (if (! strcasecmp (what, "count")) single_set_tx_count (info, value);
           ...
           else if (! strcasecmp (what, "txburst"))
               single_set_tx_burst (info, value);
           else if (! strcasecmp (what, "rxburst"))
...
set_seq (lua_State *L, uint32_t seqnum)
{
  ...
  _atoip (luaL_checkstring (L, 6), PG_IPADDR_NETWORK, &ip_saddr,
          sizeof (struct pg_ipaddr));
  ...
  foreach_port (portlist,
                pktgen_set_seq (info, seqnum, &daddr, &saddr, &ip_daddr,
                                &ip_saddr, sport, dport, ip[3], proto[0],
...
set_seqTable (lua_State *L, uint32_t seqnum)
{
  ...
  foreach_port (portlist,
                pktgen_set_seq (info, seqnum, &daddr, &saddr, &ip_daddr,
                                &ip_saddr, sport, dport, ethType[3],
                                ipProto[0], vlanid, pktSize, gtpu_teid);
                pktgen_set_cos_tos_seq (info, seqnum, cos, tos));
...
pktgen_icmp (lua_State *L)
{
  ...
  foreach_port (
      portlist,
      enable_icmp_echo (info, estate ((char *) luaL_checkstring (L, 2))));
  return 0;
}
...
pktgen_sendARP (lua_State *L)
{
  ...
  if (portlist == INVALID_PORTLIST)
    return luaL_error (L, "invalid portlist");
  foreach_port (portlist, pktgen_send_arp_requests (
                              info, (what[0] == 'g') ? GRATUITOUS_ARP : 0));
  return 0;
}
...
pktgen_set_ip_addr (lua_State *L)
{
  ...
  ip_ver = _atoip (luaL_checkstring (L, 3), flags, &ipaddr,
                   sizeof (struct pg_ipaddr));
...
/**
 *
 * pktgen_pause - Delay for a given number of milliseconds and display a
 * message
 *
 * DESCRIPTION
...
range_vlan_id (lua_State *L)
{
  ...
  foreach_port (
      portlist,
      range_set_vlan_id (info, (char *) luaL_checkstring (L, 2), vlan_id));
...
range_cos (lua_State *L)
{
  ...
  foreach_port (portlist, range_set_cos_id (
                              info, (char *) luaL_checkstring (L, 2), cos));
...
range_tos (lua_State *L)
{
  ...
  foreach_port (portlist, range_set_tos_id (
                              info, (char *) luaL_checkstring (L, 2), tos));
...
range_traffic_class (lua_State *L)
{
  ...
  foreach_port (portlist,
                range_set_traffic_class (
                    info, (char *) luaL_checkstring (L, 2), traffic_class));
...
pktgen_mpls (lua_State *L)
{
  ...
  foreach_port (portlist,
                enable_mpls (info, estate (luaL_checkstring (L, 2))));
...
pktgen_qinq (lua_State *L)
{
  ...
  foreach_port (portlist,
                enable_qinq (info, estate (luaL_checkstring (L, 2))));
...
pktgen_gre_eth (lua_State *L)
{
  ...
  foreach_port (portlist,
                enable_gre_eth (info, estate (luaL_checkstring (L, 2))));
  ...
  if (lua_gettop (L) == 3)
    {
      if (strncasecmp (lua_tostring (L, 2), "rate", 4) == 0)
        foreach_port (
            portlist,
            latency_set_rate (info, (uint32_t) luaL_checkinteger (L, 3)));
      else if (strncasecmp (lua_tostring (L, 2), "entropy", 7) == 0)
        foreach_port (
            portlist,
            latency_set_entropy (info, (uint16_t) luaL_checkinteger (L, 3)));
      else
        return luaL_error (
            L, "latency, invalid arguments must be rate or entropy");
    }
  else if (lua_gettop (L) == 2)
    foreach_port (portlist,
                  enable_latency (
                      info, estate ((const char *) luaL_checkstring (L, 2))));
...
pktgen_pattern (lua_State *L)
{
  ...
  foreach_port (portlist,
                pattern_set_type (info, (char *) luaL_checkstring (L, 2)));
...
pktgen_user_pattern (lua_State *L)
{
  ...
  foreach_port (portlist, pattern_set_user_pattern (
...
pktgen_latsampler_params (lua_State *L)
{
  ...
  foreach_port (portlist, single_set_latsampler_params (
                              info, (char *) luaL_checkstring (L, 2),
...
pktgen_latsampler (lua_State *L)
{
  ...
  foreach_port (portlist,
                pktgen_start_stop_latency_sampler (
                    info, estate ((char *) luaL_checkstring (L, 2))));
...
port_info (lua_State *L, port_info_t *info)
{
  ...
  setf_string (L, "proto_type",
               ...
               : (rte_atomic32_read (&info->port_flags) & SEND_VXLAN_PACKETS)
                   ? "VXLAN"
                   : "UDP");
  ...
  if (bus && ! strcmp (rte_bus_name (bus), "pci"))
    {
      ...
      vend[0] = device[0] = '\0';
      sscanf (rte_dev_bus_info (dev.device), "vendor_id=%4s, device_id=%4s",
              vend, device);

      rte_eth_dev_get_name_by_port (info->pid, name);
      snprintf (buff, sizeof (buff), "%d/%s:%s/%s",
                rte_dev_numa_node (dev.device), vend, device,
...
pktgen_rnd (lua_State *L)
{
  ...
  if (strcmp (msk, "off"))
    ...
    for (i = 0; (mask_idx < 32) && ((curr_bit = msk[i]) != '\0'); i++)
      if ((curr_bit == '0') || (curr_bit == '1') || (curr_bit == '.') ||
          (curr_bit == 'X') || (curr_bit == 'x'))
        mask[mask_idx++] = curr_bit;

  foreach_port (
      portlist,
      enable_random (info, pktgen_set_random_bitfield (
  ...
  for (idx = 0; idx < MAX_RND_BITFIELDS; idx++)
    {
      ...
      setf_string (L, "active",
                   (rnd_bits->active_specs & (1 << idx)) ? "Yes" : "No");
      lua_rawset (L, -3);
    }
...
pktgen_run (lua_State *L)
{
  switch (lua_gettop (L))
    ...
    default:
      return luaL_error (
          L, "run( ['cmd'|'lua'], <string_or_path>), arguments wrong.");
    case 3:
      break;
    }

  /* A cmd executes a file on the disk and can be a lua Script of command line
   * file. */
  if (strcasecmp ("cmd", luaL_checkstring (L, 1)) == 0)
    cli_execute_cmdfile (luaL_checkstring (L, 2));
  else if (strcasecmp ("lua", luaL_checkstring (L, 1)) ==
           0) /* Only a Lua script in memory. */
...
static const luaL_Reg pktgenlib_range[] = {
  /* Range commands */
  { "dst_mac",
    range_dst_mac }, /* Set the destination MAC address for a port */
...
static const luaL_Reg pktgenlib[] = {
  ...
  { "seqTable",
    pktgen_seqTable }, /* Set the sequence data for a port using tables */

  { "screen", pktgen_scrn }, /* Turn off and on the screen updates */
  { "prime",
    pktgen_prime }, /* Send a small number of packets to prime the routes */
  { "delay", pktgen_delay }, /* Delay a given number of milliseconds */
  { "pause", pktgen_pause }, /* Delay for a given number of milliseconds and
                                display message */
  { "sleep", pktgen_sleep }, /* Delay a given number of seconds */
  { "load", pktgen_load },   /* load and run a command file or Lua file. */
  { "save",
    pktgen_config_save },    /* Save the configuration of Pktgen to a file. */
  ...
  { "ports_per_page",
    pktgen_ports_per_page }, /* Set the number of ports per page */
  { "page", pktgen_page }, /* Select a page to display, seq, range, pcap and a
                              number from 0-N */
  { "port", pktgen_port }, /* select a different port number used for sequence
                              and range pages. */
  { "process",
    pktgen_process }, /* Enable or disable input packet processing on a port */
  ...
  { "isSending",
    pktgen_isSending }, /* Test to see if a port is sending packets */
  { "linkState",
    pktgen_linkState }, /* Return the current link state of a port */

  { "portSizes", pktgen_portSizes }, /* Return the stats on the size of packet
                                        for a port. */
  { "pktStats",
    pktgen_pktStats }, /* return the current packet stats on a port */
  { "portStats", pktgen_portStats }, /* return the current port stats */
  { "portInfo", pktgen_portInfo },   /* return the current port stats */

  { "run",
    pktgen_run }, /* Load a Lua string or command file and execute it. */
  { "continue", pktgen_continue }, /* Display a message and wait for keyboard
                                      key and return */
  { "input",
    pktgen_input }, /* Wait for a keyboard input line and return line. */
  ...
  { "rnd_list",
    pktgen_rnd_list }, /* Return a table of rnd bit patterns per port */
  ...
  { "clock_gettime",
    pktgen_clock_gettime }, /* Enable/disable clock_gettime support */
...
luaopen_pktgen (lua_State *L)
{
  ...
  setf_integer (L, "maxMbufsPerPort",
                MAX_MBUFS_PER_PORT (pktgen.nb_rxd, pktgen.nb_txd));
