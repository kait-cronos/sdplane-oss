...
#define MAX_LATENCY_QUEUES 10
  ...
  { /* Per port flag bits */
    ...
    SEND_GRE_ETHER_HEADER =
        (1 << 24),                  /**< Encapsulate Ethernet frame in GRE */
    ...
    SEND_ARP_PING_REQUESTS = (SEND_ARP_REQUEST | SEND_GRATUITOUS_ARP |
                              SEND_PING4_REQUEST | SEND_PING6_REQUEST)
  };

#define EXCLUSIVE_MODES                                                       \
  (SEND_PCAP_PKTS | SEND_RANGE_PKTS | SEND_SEQ_PKTS | SEND_RANDOM_PKTS |      \
   SEND_RATE_PACKETS)
...
#define RTE_PMD_PARAM_UNSET -1
  ...
  {
    uint64_t
        latency_rate_us; /**< number micro-seconds between injecting packets */
    ...
    uint64_t
        latency_timo_cycles; /**< Number of cycles to next latency injection */
    uint16_t
        latency_entropy;   /**< Entropy value to be used to increment sport */
    ...
    uint64_t
        running_cycles;   /**< Running, Number of cycles per latency packet */
    uint64_t prev_cycles; /**< previous cycles cyles time from last latency
                             packet */
  ...
  typedef struct port_info_s
  {
    ...
    /* Packet buffer space for traffic generator, shared for all packets per
     * port */
    uint16_t seqIdx; /**< Current Packet sequence index 0 to NUM_SEQ_PKTS */
    uint16_t seqCnt; /**< Current packet sequence max count */
    uint16_t
        prime_cnt; /**< Set the number of packets to send in a prime command */
    uint16_t vlanid; /**< Set the port VLAN ID value */
    uint8_t cos;     /**< Set the port 802.1p cos value */
    uint8_t tos;     /**< Set the port tos value */
    rte_spinlock_t
        port_lock; /**< Used to sync up packet constructor between cores */
    pkt_seq_t *seq_pkt; /**< Sequence of packets seq_pkt[NUM_SEQ_PKTS]=default
                           packet */
    ...
    struct q_info
    {
      ...
      struct rte_mempool
          *range_mp; /**< Pool pointer for port Range TX mbufs */
      struct rte_mempool
          *seq_mp; /**< Pool pointer for port Sequence TX mbufs */
    ...
    /* Depending on MAX_LATENCY_ENTRIES, this could blow up static array memory
     * usage over the limit allowed by x86_64 architecture */
    ...
    do
      {
        ...
        success = rte_atomic64_cmpset ((volatile uint64_t *) &v->cnt, tmp1,
                                       tmp1 - tmp2);
      }
    while (success == 0);
  ...
  rte_eth_rxconf_dump (FILE *f, struct rte_eth_rxconf *rx)
  {
    ...
    fprintf (f,
             ...
             " wthresh        :%5" PRIu16 "\n",
             rx->rx_thresh.pthresh, rx->rx_thresh.hthresh,
  ...
  rte_eth_txconf_dump (FILE *f, struct rte_eth_txconf *tx)
  {
    ...
    fprintf (f,
             "     pthresh        :%5" PRIu16 " hthresh          :%5" PRIu16
             " wthresh        :%5" PRIu16 "\n",
             tx->tx_thresh.pthresh, tx->tx_thresh.hthresh,
  ...
  static inline void
  rte_eth_dev_portconf_dump (FILE *f, struct rte_eth_dev_portconf *conf,
                             int tx_flag)
  {
...
#undef _
  ...
  rte_eth_dev_info_dump (FILE *f, uint16_t pid)
  {
    ...
    fprintf (f,
             ...
             ") **\n",
             dev_name, di->if_index, *di->dev_flags);
    ...
    fprintf (f, "   rx_queue_offload_capa :%016" PRIx64,
