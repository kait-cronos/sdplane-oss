...
pktgen_page_seq (uint32_t pid)
{
  ...
  row = PORT_STATE_ROW;
  col = 1;
  scrn_printf (row, col, "Port: %2d, Sequence Count: %2d of %2d  ", pid,
               info->seqCnt, NUM_SEQ_PKTS);
  pktgen_display_set_color ("stats.stat.label");
  scrn_printf (row++, col + 97, "GTP-u");
  scrn_printf (row++, col, "%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s", 3, "Seq", 15,
               "Dst MAC", 15, "Src MAC", 16, "Dst IP", 18, "Src IP", 13,
               "TTL/Port S/D", 16, "Protocol:VLAN", 8, "CoS/ToS", 5, "TEID",
               17, "Flag/Group/vxlan", 6, "Size");
  ...
  for (i = 0; i < info->seqCnt; i++)
    {
      ...
      col += 3;
      snprintf (
          buff, sizeof (buff), "%02x%02x:%02x%02x:%02x%02x",
          pkt->eth_dst_addr.addr_bytes[0], pkt->eth_dst_addr.addr_bytes[1],
          pkt->eth_dst_addr.addr_bytes[2], pkt->eth_dst_addr.addr_bytes[3],
          pkt->eth_dst_addr.addr_bytes[4], pkt->eth_dst_addr.addr_bytes[5]);
      ...
      col += 18;
      snprintf (buff, sizeof (buff), "%d/%d/%d", pkt->ttl, pkt->sport,
                pkt->dport);
