  ...
  for (i = 0; i < pktgen.nb_ports; i++)
    {
      ...
      fprintf (
          fd,
          "# Port: %2d, Burst (Rx/Tx):%3d/%3d, Rate:%g%%, Flags:%08x, TX Count:%s\n",
          info->pid, info->rx_burst, info->tx_burst, info->tx_rate, flags,
          buff);
      ...
      fprintf (fd, "set %d pattern %s\n", i,
               ...
               : (info->fill_pattern_type == ZERO_FILL_PATTERN) ? "zero"
                                                                : "user");
      if ((info->fill_pattern_type == USER_FILL_PATTERN) &&
          strlen (info->user_pattern))
        {
          char buff[64];
      ...
      for (j = 0; j < info->seqCnt; j++)
        {
          ...
          fprintf (fd, "%d %d %s %s %d %d %d\n", pkt->sport, pkt->dport,
                   ...
                   : (pkt->ipProto == PG_IPPROTO_ICMP) ? "icmp"
                                                       : "udp",
                   pkt->vlanid, pkt->pktSize + RTE_ETHER_CRC_LEN,
                   pkt->gtpu_teid);
        }
      ...
      if (info->seqCnt)
        {
          fprintf (
              fd,
              "-- (seqnum, port, dst_mac, src_mac, ip_dst, ip_src, sport, dport, "
...
pktgen_link_state (int port, char *buff, int len)
{
  ...
  if (info->link.link_status)
    snprintf (buff, len, "<UP-%u-%s>", (uint32_t) info->link.link_speed,
              (info->link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX) ? ("FD")
                                                                   : ("HD"));
  else
...
/**
 *
 * pktgen_transmit_count_rate - Get a string for the current transmit count and
 * rate
 *
 * DESCRIPTION
...
pktgen_transmit_count_rate (int port, char *buff, int len)
{
  ...
  if (rte_atomic64_read (&info->transmit_count) == 0)
    ...
    snprintf (buff, len, "%" PRIu64 " /%g%%",
              rte_atomic64_read (&info->transmit_count), info->tx_rate);
...
/**
 *
 * rate_transmit_count_rate - Get a string for the current transmit count and
 * rate
 *
 * DESCRIPTION
 * Current value of the transmit count/%rate as a string.
...
/**
 *
 * pktgen_set_port_number - Set the current port number for sequence and range
 * pages
 *
 * DESCRIPTION
...
{
  printf (
      "  %d: rate %.2f, tx_cycles %'ld, tx_pps %'ld, link %s-%d-%s, hz %'ld\n",
...
pktgen_start_latency_sampler (port_info_t *info)
{
  ...
  if (rxq == 0 || rxq > MAX_LATENCY_QUEUES)
    {
      pktgen_log_error (
          "no rx queues or rx queues over limit (%d) to sample on this port!",
          MAX_LATENCY_QUEUES);
      return;
    }
...
pktgen_stop_latency_sampler (port_info_t *info)
{
  ...
  if (outfile == NULL)
  ...
  else
    {
      ...
      fclose (outfile);
      pktgen_log_warning ("Wrote %d sample latencies to file %s", count,
                          info->latsamp_outfile);
    }
...
/**
 *
 * pktgen_prime_ports - Send a small number of packets to setup forwarding
 * tables
 *
 * DESCRIPTION
...
{
  info->seq_pkt[SINGLE_PKT].ipProto =
      (type[0] == 'u')   ? PG_IPPROTO_UDP
...
{
  info->seq_pkt[RANGE_PKT].ipProto =
      (type[0] == 'u')   ? PG_IPPROTO_UDP
      : (type[0] == 'i') ? PG_IPPROTO_ICMP
      : (type[0] == 't') ? PG_IPPROTO_TCP
                         :
...
pcap_filter (port_info_t *info, char *str)
{
  ...
  info->pcap_result =
      pcap_compile (pc, &info->pcap_program, str, 1, PCAP_NETMASK_UNKNOWN);
...
enable_bonding (port_info_t *info, uint32_t state)
{
  ...
  num_workers =
      rte_eth_bond_members_get (info->pid, workers, RTE_MAX_ETHPORTS);
  if (num_workers < 0)
    {
  ...
  num_active_workers = rte_eth_bond_active_members_get (
      info->pid, active_workers, RTE_MAX_ETHPORTS);
...
{
  const char *states[] = { "LACP_Active", "LACP_Short_timeout",
...
show_bonding_mode (port_info_t *info)
{
  ...
  num_active_workers =
      rte_eth_bond_active_members_get (port_id, workers, RTE_MAX_ETHPORTS);
...
{
  info->seq_pkt[RANGE_PKT].ethType =
...
single_set_pkt_type (port_info_t *info, const char *type)
{
  ...
  if ((ethtype == RTE_ETHER_TYPE_IPV6) &&
...
void
single_set_latsampler_params (port_info_t *info, char *type,
                              uint32_t num_samples, uint32_t sampling_rate,
                              char outfile[])
{
  FILE *fp = NULL;
  ...
  if (pktgen_tst_port_flags (info, SAMPLING_LATENCIES))
    {
      pktgen_log_warning (
          "Latency sampler is already running, stop it first!");
      return;
    }
  ...
  if (! strcasecmp (type, "simple"))
    ...
    {
      pktgen_log_error (
          "Unknown latsampler type %s! Valid values: simple, poisson", type);
      return;
    }
  ...
  if (num_samples > MAX_LATENCY_ENTRIES)
    {
      pktgen_log_error ("Too many samples requested. Max %d!",
                        MAX_LATENCY_ENTRIES);
      return;
    }
...
pktgen_port_defaults (uint32_t pid, uint8_t seq)
{
  ...
  if (dst_info->seq_pkt != NULL)
    {
      rte_ether_addr_copy (&dst_info->seq_pkt[SINGLE_PKT].eth_src_addr,
                           &pkt->eth_dst_addr);
      rte_ether_addr_copy (&dst_info->seq_pkt[RATE_PKT].eth_src_addr,
                           &pkt->eth_dst_addr);
      rte_ether_addr_copy (&dst_info->seq_pkt[LATENCY_PKT].eth_src_addr,
                           &pkt->eth_dst_addr);
    }
  else
...
{
  memcpy (&info->seq_pkt[PING_PKT], &info->seq_pkt[SINGLE_PKT],
          sizeof (pkt_seq_t));
...
debug_pdump (port_info_t *info)
{
  ...
  *ppkt = *spkt; /* Copy the sequence setup to the dump setup. */
  ...
  rte_memcpy ((uint8_t *) m->buf_addr + m->data_off, (uint8_t *) &ppkt->hdr,
              ppkt->pktSize);
...
pktgen_port_restart (port_info_t *info)
{
  ...
  /* Stop and start the device to flush TX and RX buffers from the device
   * rings. */
  ...
  if (ip_ver == 4)
    {
      ...
      if (pkt->ethType != RTE_ETHER_TYPE_IPV4 &&
          pkt->ethType != RTE_ETHER_TYPE_ARP)
...
void
single_set_mac (port_info_t *info, const char *which,
  ...
  if (state == ENABLE_STATE)
    {
      if (pktgen_tst_port_flags (info, SENDING_PACKETS))
        {
          pktgen_log_warning (
              "Cannot enable the range settings while sending packets!");
          return;
        }
      pktgen_clr_port_flags (info, EXCLUSIVE_MODES);
...
void
range_set_dest_mac (port_info_t *info, const char *what,
                    struct rte_ether_addr *mac)
{
...
pktgen_set_seq (port_info_t *info, uint32_t seqnum,
                struct rte_ether_addr *daddr, struct rte_ether_addr *saddr,
...
void
pktgen_set_cos_tos_seq (port_info_t *info, uint32_t seqnum, uint32_t cos,
                        uint32_t tos)
{
  pkt_seq_t *pkt;
...
void
pktgen_set_vxlan_seq (port_info_t *info, uint32_t seqnum, uint32_t flag,
                      uint32_t gid, uint32_t vid)
{
  pkt_seq_t *pkt;
