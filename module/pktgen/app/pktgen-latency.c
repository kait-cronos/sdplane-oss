  ...
  for (pid = 0; pid < pktgen.nb_ports_per_page; pid++)
    {
      ...
      snprintf (buff, sizeof (buff), "%'" PRIu64 "/%'8.2f", lat->min_cycles,
                latency);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);

      latency = cycles_to_us (lat->avg_cycles, per_cycle);
      snprintf (buff, sizeof (buff), "%'" PRIu64 "/%'8.2f", lat->avg_cycles,
                latency);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);

      latency = cycles_to_us (lat->max_cycles, per_cycle);
      snprintf (buff, sizeof (buff), "%'" PRIu64 "/%'8.2f", lat->max_cycles,
                latency);
      scrn_printf (row++, col, "%*s", COLUMN_WIDTH_1, buff);
...
pktgen_latency_setup (port_info_t *info)
{
  ...
  rte_memcpy (&info->seq_pkt[LATENCY_PKT], &info->seq_pkt[SINGLE_PKT],
              sizeof (pkt_seq_t));
