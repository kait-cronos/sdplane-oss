...
extern "C"
{
  ...
  /**
   * The function is a wrapper around strdup() and will free the previous
   * string if the pointer is present.
   */
  ...
  /**
   * Trim a set of characters like "[]" or "{}" from the start and end of
   * string.
   *
   * @param str
   ...
   *   remove then you must call the routine for each set. The <set> string
   * must be two characters and can be any characters you want to call a set.
   * @return
  ...
  uint32_t pg_strparse (char *s, const char *delim, char **entries,
