    ...
    for (i = 0; i < RTE_MAX_ETHPORTS; i++)
      {
        ...
        if (pobj->nb_lids == 0)
          continue;
        printf ("  Lids idx %2d: RX %2d, TX %2d, private %p: ", pobj->pid,
                pobj->rx_qid, pobj->tx_qid, pobj->private);
            ...
            if (lobj->pids.rx_cnt)
              {
                ...
                for (i = 0; i < lobj->pids.rx_cnt; i++)
                  printf ("(pid=%2d:qid=%2d) ", lobj->pids.rx[i],
            ...
            if (lobj->pids.tx_cnt)
              {
                printf ("                TX_cnt(%2d): ", lobj->pids.tx_cnt);
                for (i = 0; i < lobj->pids.tx_cnt; i++)
                  printf ("(pid=%2d:qid=%2d) ", lobj->pids.tx[i],
    ...
    for (pid = 0; pid < RTE_MAX_ETHPORTS; pid++)
      {
        ...
        if (pobj->nb_lids)
          {
            printf ("   %2d, nb_lcores %2d, private %p,", pobj->pid,
                    pobj->nb_lids, pobj->private);
