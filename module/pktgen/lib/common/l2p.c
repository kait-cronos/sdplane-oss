...
pg_parse_lcore_list (char *list, ls_t *ls)
{
  ...
  if (k == 1)
    { /* Must be a lcore/port number only */
      pg_parse_rt_list (arr[0],
                        ls->rbits); /* Parse the list with no ':' character */
      memcpy (ls->tbits, ls->rbits, sizeof (ls->rbits));
    }
  else
    { /* k == 2 Must be a <rx-list>:<tx-list> pair */
      if (pg_parse_rt_list (arr[0], ls->rbits))
        { /* parse <rx-list> */
          fprintf (stderr, "*** Unable to parse rx-list\n");
          return 1;
        }

      if (pg_parse_rt_list (arr[1], ls->tbits))
        { /* parse <tx-list> */
...
pg_parse_port_list (char *list, ps_t *ps)
{
  ...
  if (k == 1)
    { /* Must be a lcore/port number only */
      pg_parse_rt_list (arr[0],
                        ps->rbits); /* Parse the list with no ':' character */
      pg_parse_rt_list (arr[0],
                        ps->tbits); /* Parse the list with no ':' character */
    }
  else
    { /* k == 2 Must be a <rx-list>:<tx-list> pair */
      if (pg_parse_rt_list (arr[0], ps->rbits))
        { /* parse <rx-list> */
          fprintf (stderr, "%s: *** parse <rx-list] (%s)\n", __func__, arr[0]);
          return 1;
        }

      if (pg_parse_rt_list (arr[1], ps->tbits))
        { /* parse <tx-list> */
 ...
 *	[1:2-3].0, [4:5-6].1, ...	- core 1 handles port 1 rx, cores 2,3
 *handle port 0 tx core 4 handles port 1 rx & core 5,6 handles port 1 tx
 *	[1-2:3].0, [4-5:6].1, ...	- core 1,2 handles port 0 rx, core 3
 *handles port 0 tx core 4,5 handles port 1 rx & core 6 handles port 1 tx
 *	[1-2:3-5].0, [4-5:6/8].1, ...	- core 1,2 handles port 0 rx, core
 *3,4,5 handles port 0 tx core 4,5 handles port 1 rx & core 6,8 handles port 1
 *tx [1:2].[0:0-7], [3:4].[1:0-7],   - core 1 handles port 0 rx, core 2 handles
 *ports 0-7 tx core 3 handles port 1 rx & core 4 handles port 0-7 tx BTW: you
 *can use "{}" instead of "[]" as it does not matter to the syntax.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
