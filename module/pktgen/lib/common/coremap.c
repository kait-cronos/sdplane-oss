...
set_thread_id_str (const char *unused, lcore_t *lc)
{
  (void) unused;
  lc->u.s.thread_id =
      get_next_thread_id (lc->next, lc->u.s.socket_id, lc->u.s.core_id);
  return lc;
}
  ...
  if (lc)
    {
      ...
      printf ("%u/%u/%u\t%u\n", lc->u.s.socket_id, lc->u.s.core_id,
              lc->u.s.thread_id, lc->u.s.id);
      free (lc);
    }
}
...
int
coremap (const char *arg, lc_info_t *get_lcores, int max_cnt,
         const char *proc_cpuinfo)
{
  ...
  printf (
      "%u socket%s, %u core%s per socket and %u thread%s per core\n",
...
{
  _getter_fn get;
  _getter_fn _type[] = { _get_socket_id, _get_core_id, _get_thread_id,
                         _get_lcore_id, NULL };
  unsigned cnt = 0, i;
