...
#define GTPu_PN_FLAG 0x01
  ...
  typedef struct gtpuHdr_s
  {
    ...
    uint16_t tot_len;
    uint32_t teid;
    //	uint16_t seq_no;		/**< Optional fields if E, S or PN
    // flags set */ 	uint8_t npdu_no; 	uint8_t next_ext_hdr_type;
...
#define MPLS_CLR_BOS(mpls_label)                                              \
   ...
   * Protocol Type: EtherType of encapsulated packet. When IPv4 is being
   * carried as the GRE payload, the Protocol Type field MUST be set to 0x800.
   * Checksum: the IP (one's complement) checksum sum of all the 16 bit words
   * in the GRE header and the payload packet Reserved1: must be 0 Key: 32bit
   * number determined by the encapsulator Sequence Number: for packet ordering
   * purposes
   */
  ...
  typedef struct greHdr_s
  {
    ...
    uint32_t extra_fields[3]; /**< Room for Checksum+Reserved1, Key and
                                 Sequence Number fields if present */
...
#define ETHER_TYPE_TRANSP_ETH_BR 0x6558 /* Transparent Ethernet Bridge */
  ...
  rte_hash6_crc (const void *data, __attribute__ ((unused)) uint32_t data_len,
                 uint32_t init_val)
  {
#ifdef RTE_MACHINE_CPUFLAG_SSE4_2
  ...
  inet6AddrAdd (void *paddr1, void *paddr2, void *paddr3)
  {
    ...
    uint8_t *addr1 = (uint8_t *) paddr1, *addr2 = (uint8_t *) paddr2,
...
#ifndef _NTOP4_
#define _NTOP4_
  /* char * inet_ntop4(char * buff, int len, unsigned long ip_addr, unsigned
   * long mask) - Convert IPv4 address to ascii */
  ...
  /* const char * inet_ntop6(char * buff, int len, uint8_t *ip6, unsigned
   * prefixlen) - Convert IPv6 address to ascii */
    ...
    if (prefixlen > PG_PREFIXMAX)
      {
        ...
        if (nprint < strlen (lbuf) + ntrim)
          {
            nprint -= ntrim;
            snprintf (buff, len, "%.*s+%02ld", nprint - 3, lbuf,
                      strlen (lbuf) - nprint + 3);
          }
      }
...
#ifndef _MTOA_
#define _MTOA_
  /* char * inet_mtoa(char * buff, int len, struct rte_ether_addr * eaddr) -
   * Convert MAC address to ascii */
  static __inline__ char *
  inet_mtoa (char *buff, int len, struct rte_ether_addr *eaddr)
  {
    snprintf (buff, len, "%02x:%02x:%02x:%02x:%02x:%02x", eaddr->addr_bytes[0],
