...
_pcap_open (char *filename, uint16_t port)
{
  ...
  pcap = (pcap_info_t *) rte_malloc_socket ("PCAP info", sizeof (pcap_info_t),
                                            RTE_CACHE_LINE_SIZE,
                                            rte_socket_id ());
  if (pcap == NULL)
    {
  ...
  if (fread (&pcap->info, 1, sizeof (pcap_hdr_t), pcap->fd) !=
      sizeof (pcap_hdr_t))
    {
...
_pcap_info (pcap_info_t *pcap, uint16_t port, int flag)
{
  ...
  printf (" Version: %d.%d,", pcap->info.version_major,
...
_pcap_skip (pcap_info_t *pcap, uint32_t skip)
{
  ...
  while (skip--)
    {
      if (fread (phdr, 1, sizeof (pcaprec_hdr_t), pcap->fd) !=
                     ...
                     unsigned int *length)
{
  ...
  *length =
      sizeof (struct rte_ether_hdr) + ntohs (iph->total_length) - *offset;
...
size_t
_pcap_read (pcap_info_t *pcap, pcaprec_hdr_t *pHdr, char *pktBuff,
            uint32_t bufLen)
{
  do
    {
