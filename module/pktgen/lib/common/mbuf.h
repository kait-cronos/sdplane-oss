...
extern "C"
{
   ...
   */
  static inline int
  pg_pktmbuf_alloc_bulk (struct rte_mempool *pool, struct rte_mbuf **mbufs,
                         unsigned count)
  {
    unsigned idx;
    int rc;
  ...
  pg_pktmbuf_dump (FILE *f, const struct rte_mbuf *m, unsigned dump_len)
  {
    ...
    fprintf (f, "dump mbuf at %p, addr=%p iova=%" PRIx64 ", buf_len=%u\n",
             (void *) (uintptr_t) m, m->buf_addr, (uint64_t) m->buf_iova,
             (unsigned) m->buf_len);
    fprintf (f, "  offset=%d, refcnt=%d, len=%u, mp=%p\n", m->data_off,
             m->refcnt, m->data_len, m->pool);
    fprintf (f, "  pkt_len=%" PRIu32 ", nb_segs=%u, in_port=%u, next %p\n",
             m->pkt_len, (unsigned) m->nb_segs, (unsigned) m->port, m->next);
    ...
    while (m && nb_segs != 0)
      {
        ...
        fprintf (f, "  segment at %p, data=%p, data_len=%u\n",
                 (void *) (uintptr_t) m, rte_pktmbuf_mtod (m, void *),
                 (unsigned) m->data_len);
        len = dump_len;
        if (len > m->data_len)
          len = m->data_len;
