...
uint32_t
get_portdesc (struct rte_pci_addr *pciAddr, uint8_t **portdesc, uint32_t num,
              int verbose)
{
  FILE *fd;
  uint32_t idx;
      ...
      if (portdesc)
        {
          ...
          if (p) /* Try to remove the two strings to make the line shorter */
            memmove (p, p + n, (strlen (buff) - ((p - buff) + n)) + 1);
          portdesc[idx] = (uint8_t *) strdup (
              buff); /* portdesc[idx] needs to be NULL or we lose memory. */
        }
      if (++idx >= num)
        break;
    }
...
uint32_t
create_blocklist (uint64_t portmask, struct rte_pci_addr *portlist,
                  uint32_t port_cnt, uint8_t *desc[])
{
  ...
  fprintf (stdout,
           "Ports: Port Mask: %016" PRIx64
           " blocklisted = --, not-blocklisted = ++\n",
           portmask);
  idx = 0;
