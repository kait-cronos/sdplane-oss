...
_tostring (lua_State *L)
{
  ...
  snprintf (buff, sizeof (buff), "%s<%s,%d,%d,%d,%d>", lua_tostring (L, -1),
            mp->name, mp->size, mp->elt_size, mp->cache_size, mp->socket_id);
  lua_pop (L, 3);
...
static const struct luaL_Reg _methods[] = { { "get", _get },
                                            { "put", _put },
                                            { NULL, NULL } };

static const struct luaL_Reg _functions[] = { { "new", _new },
                                              { "destroy", _destroy },
                                              { NULL, NULL } };
