...
link_state (lua_State *L, uint16_t pid)
{
  ...
  if (link.link_status)
    snprintf (buff, sizeof (buff), "<UP-%u-%s>", (uint32_t) link.link_speed,
              (link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX) ? ("FD")
                                                             : ("HD"));
  else
  ...
  if (p->ethType == RTE_ETHER_TYPE_IPV4)
    {
      ...
      setf_string (L, "ip_src_addr",
                   inet_ntop4 (buff, sizeof (buff), htonl (p->ip_src_addr),
                               p->ip_mask));
    }
...
static const luaL_Reg dpdklib[] = {
  ...
  { "pause", dpdk_pause }, /* Delay for a given number of milliseconds and
                              display message */
  ...
  { "linkState",
    dpdk_linkState }, /* Return the current link state of a port */

  { "compile",
    dpdk_compile }, /* Convert a structure into a frame to be sent */
  { "decompile", dpdk_decompile }, /* decompile a frame into Ethernet, IP, TCP,
...
static const char *rte_copyright =
