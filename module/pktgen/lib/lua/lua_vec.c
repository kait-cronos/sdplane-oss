...
vec_new (lua_State *L)
{
  ...
  v = (struct vec *) lua_newuserdata (
      L, (sizeof (struct vec) * (size * sizeof (void *))));
...
vec_tostring (lua_State *L)
{
  ...
  snprintf (buff, sizeof (buff), "%s<%d,%d,%s,0x%04x>", lua_tostring (L, -1),
...
static const struct luaL_Reg vec_methods[] = { { "add1", lvec_add1 },
                                               { NULL, NULL } };

static const struct luaL_Reg vec_functions[] = { { "new", vec_new },
                                                 { NULL, NULL } };
