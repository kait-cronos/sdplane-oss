...
_parse_ether (fgen_t *fg, frame_t *f, int lidx)
{
  ...
  rte_ether_unformat_addr ("FFFF:FFFF:FFFF",
                           (struct rte_ether_addr *) eth->ether_dhost);
  rte_ether_unformat_addr ("0000:0000:0000",
                           (struct rte_ether_addr *) eth->ether_shost);
...
_parse_ipv4 (fgen_t *fg, frame_t *f, int lidx)
{
  ...
  switch (nxt)
    ...
    case FGEN_UDP_TYPE:
      ...
      udp =
          (struct rte_udp_hdr *) ((char *) hdr + (hdr->version_ihl & 0xf) * 4);
      udp->dgram_cksum = 0;
    ...
    case FGEN_TCP_TYPE:
      ...
      tcp =
          (struct rte_tcp_hdr *) ((char *) hdr + (hdr->version_ihl & 0xf) * 4);
...
_parse_payload (fgen_t *fg, frame_t *f, int lidx)
{
  ...
  for (int i = 0; i < num; i++)
    {
      switch (parser_kvp (fg->params[i], kvps, RTE_DIM (kvps), &val))
        ...
        case 0: /* Force the frame size to a given length */
          ...
          /* fsize is the absolute packet length even if current packet is
           * greater */
          if (fsize != pktlen)
            pktlen = fsize;
          break;
  ...
  if ((fgen_data_len (f) != pktlen) && (fill != 0))
    memset (fgen_mtod_offset (f, char *, pktlen), fill,
            fgen_data_len (f) - pktlen);
  ...
  if (fgen_data_len (f) < ETH_ZLEN)
    {
      if (fg->flags & FGEN_VERBOSE)
        _WARN (
            "[magenta]Packet is to short [orange]%d[], [magenta]adjusting to [orange]%d "
            "[magenta]bytes[]\n",
  ...
  if (fgen_data_len (f) > ETH_FRAME_LEN)
    {
      if (fg->flags & FGEN_VERBOSE)
        _WARN (
            "[magenta]Packet is to long [orange]%d[], [magenta]adjusting to [orange]%d "
            "[magenta]bytes[]\n",
