...
int
fgen_alloc (fgen_t *fg, int low, int high, struct rte_mbuf **mbufs,
            uint32_t nb_pkts)
{
  frame_t *f;
  void *pkt_data;
  ...
  for (uint32_t i = 0; i < nb_pkts; i++)
    {
      ...
      /* increment tsc by one cycle for each packet, just to have different
       * values in each. */
      if (f->tsc_off)
  ...
  for (;;)
    {
      ...
      /* Trim off any comments in the line, which could leave nothing in the
       * buffer */
      ...
      /* Check to see if we have a ':=' string in the line, which means we have
       * read too much */
      c = strstr (p, ":=");
      if (c)
        {
      ...
      /* This line must be part of the current fgen text line, append the
       * string */
