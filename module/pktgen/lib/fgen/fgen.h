...
#define _ERR_GOTO(lbl, ...)                                                   \
  ...
  enum
  {
    FGEN_MAX_STRING_LENGTH =
        1024,             /**< Maximum string length for text string */
    FGEN_MAX_LAYERS = 32, /**< Maximum number of layers in the text string */
    FGEN_MAX_PARAMS =
        16, /**< Maximum number of parameters in the text string */
    FGEN_MAX_KVP_TOKENS =
        4, /**< Maximum number of tokens in a key/value pair + 1 */
...
#define FGEN_TYPE_STRINGS                                                     \
  {                                                                           \
    "Ether", "Dot1q", "Dot1ad", "IPv4", "IPv6", "UDP", "TCP", "Vxlan",        \
        "Echo", "TSC", "Raw", "Payload", "Done"                               \
  }
...
#define FGEN_DONE_TYPE FGEN_TYPE_COUNT /**< A parsing done flag */
  ...
  typedef struct fgen_s
  {
    ...
    fopt_t opts[FGEN_MAX_LAYERS];  /**< The option information one for each
                                      layer */
...
#define fgen_mtod(f, t) fgen_mtod_offset (f, t, 0)
  ...
  /**
   * Create a frame generator object with number of frames and size of each
   * frame.
   *
   * @param max_frames
   *   The maximum number of frames allowed to be added to the fgen_t
   * structure.
   * @param frame_sz
  ...
  /**
   * Copy the frame generator object into the list of mbufs and adjust the
   * mbufs accordingly.
   *
   * @param fg
   *   The fgen_t pointer returned from fgen_create()
   ...
   * @param mbufs
   *   The mbuf array to copy data and the caller must supply the mbufs array
   * pointers.
   * @param nb_pkts
   ...
   *   0 on success or -1 on error.
   */
  int fgen_alloc (fgen_t *fg, int low, int high, struct rte_mbuf **mbufs,
                  uint32_t nb_pkts);
