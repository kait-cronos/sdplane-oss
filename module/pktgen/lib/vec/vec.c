...
static void
vec_obj_init (struct rte_mempool *mp, void *uarg __rte_unused, void *obj,
              unsigned idx __rte_unused)
{
...
struct rte_mempool *
vec_create_pool (const char *name, unsigned int n, unsigned int entires,
                 unsigned int cache_size)
{
  struct rte_mempool *mp = NULL;
  unsigned int size = vec_calc_size (entires);

  mp = rte_mempool_create (name, n, size, cache_size, 0, NULL, NULL, NULL,
...
vec_create (const char *name, unsigned int n, uint16_t flags)
{
  ...
  vec = rte_zmalloc_socket (name, vec_size, RTE_CACHE_LINE_SIZE,
                            rte_socket_id ());
  if (vec)
    {
      vec->tlen = n;
