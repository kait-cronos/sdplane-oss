...
static TAILQ_HEAD (plugin_list,
                   plugin) plugin_head = TAILQ_HEAD_INITIALIZER (plugin_head);
...
plugin_create (char *plugin, char *path)
{
      ...
      if (pin && ! strcmp (path, pin->path))
        {
          ...
          PLUGIN_LOG (DEBUG, "Plugin: '%s' - %s, version: %d.%02d.%02d-rc%d\n",
                      pin->plugin, info->desc, info->major, info->minor,
                      info->patch, info->release);
          return inst;
        }
      /* not the same plugin */
    }
  ...
  PLUGIN_LOG (DEBUG, "Plugin: '%s' - %s, version: %d.%02d.%02d-rc%d\n",
              pin->plugin, info->desc, info->major, info->minor, info->patch,
              info->release);
