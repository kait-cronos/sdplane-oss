...
key_ctrl_k (void)
{
  ...
  if (gb_right_data_size (cli->gb))
    cli->kill =
        strndup (gb_end_of_gap (cli->gb), gb_right_data_size (cli->gb) + 1);
...
// This is the 'super key' which has had many usages in the past. I changed it
// to but the back space key as it seems windows or mobaxterm sends this code
// to delete key.
static inline void
key_suppr (void)
{
