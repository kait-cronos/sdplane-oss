...
__add_node (const char *name, struct cli_node *parent, int type,
            cli_funcs_t func, const char *short_desc)
{
  struct cli_node *node;
...
struct cli_node *
cli_add_cmd (const char *name, struct cli_node *dir, cli_cfunc_t func,
...
struct cli_node *
cli_add_alias (const char *name, struct cli_node *dir, const char *line,
               const char *short_desc)
{
...
struct cli_node *
cli_add_file (const char *name, struct cli_node *dir, cli_ffunc_t func,
              const char *short_desc)
{
  cli_funcs_t funcs;
...
cli_execute (void)
{
  ...
  switch (node->type)
    ...
    case CLI_CMD_NODE:
      /*
       * Reset global optind so getopt works as expected in a node's command
       * function. The getopt man page says to set optind to 0 instead of 1 if
       * a program scans multiple argument vectors, which can happen if
       * multiple commands use getopt.
       */
      optind = 0;
