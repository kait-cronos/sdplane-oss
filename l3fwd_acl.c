...
static struct rte_acl_rule *acl_base_ipv4, *route_base_ipv4, *acl_base_ipv6,
    *route_base_ipv6;
static unsigned int acl_num_ipv4, route_num_ipv4, acl_num_ipv6, route_num_ipv6;
...
parse_cb_ipv4vlan_rule (char *str, struct rte_acl_rule *v, int has_userdata)
{
  ...
  rc =
      parse_ipv4_net (in[CB_FLD_SRC_ADDR], &v->field[SRC_FIELD_IPV4].value.u32,
  ...
  rc =
      parse_ipv4_net (in[CB_FLD_DST_ADDR], &v->field[DST_FIELD_IPV4].value.u32,
...
static int
acl_add_rules (const char *rule_path, struct rte_acl_rule **proute_base,
...
usage_acl_alg (char *buf, size_t sz)
{
  ...
  for (i = 0; i < RTE_DIM (acl_alg); i++)
    {
      rc =
          snprintf (buf + n, sz - n, i == RTE_DIM (acl_alg) - 1 ? "%s" : "%s|",
                    acl_alg[i].name);
      tn += rc;
      if (rc < sz - n)
        n += rc;
...
app_acl_init (struct rte_acl_rule *route_base, struct rte_acl_rule *acl_base,
              unsigned int route_num, unsigned int acl_num, int ipv6,
              int socketid)
{
  char name[PATH_MAX];
  ...
  if (parm_config.rule_ipv6_name != NULL)
    {
      if (acl_add_rules (parm_config.rule_ipv6_name, &route_base_ipv6,
                         &route_num_ipv6, &acl_base_ipv6, &acl_num_ipv6,
...
print_one_ipv4_rule (struct acl4_rule *rule, int extra)
{
  ...
  printf ("%s/%u ", inet_ntop (AF_INET, &(ipv4_addr), abuf, sizeof (abuf)),
...
dump_acl4_rule (struct rte_mbuf *m, uint32_t sig)
{
  ...
  printf ("Packet Src:%s ",
          inet_ntop (AF_INET, ipv4_hdr->src_addr, abuf, sizeof (abuf)));
  printf ("Dst:%s ",
          inet_ntop (AF_INET, ipv4_hdr->dst_addr, abuf, sizeof (abuf)));
...
dump_acl6_rule (struct rte_mbuf *m, uint32_t sig)
{
  ...
  printf ("Packet Src");
  printf ("%s", inet_ntop (AF_INET6, ipv6_hdr->src_addr, abuf, sizeof (abuf)));
  printf ("\nDst");
  printf ("%s", inet_ntop (AF_INET6, ipv6_hdr->dst_addr, abuf, sizeof (abuf)));
...
setup_acl (const int socket_id)
{
  ...
  acl_config.acx_ipv4[socket_id] =
      app_acl_init (route_base_ipv4, acl_base_ipv4, route_num_ipv4,
                    acl_num_ipv4, 0, socket_id);

  acl_config.acx_ipv6[socket_id] =
      app_acl_init (route_base_ipv6, acl_base_ipv6, route_num_ipv6,
                    acl_num_ipv6, 1, socket_id);
              ...
              if (acl_search.num_ipv4)
                {
                  ...
                  l3fwd_acl_send_packets (qconf, pkts_burst,
                                          acl_search.res_ipv4, nb_rx);
                }

              if (acl_search.num_ipv6)
                {
                  rte_acl_classify (acl_config.acx_ipv6[socketid],
                                    acl_search.data_ipv6, acl_search.res_ipv6,
                                    acl_search.num_ipv6,
                                    DEFAULT_MAX_CATEGORIES);

                  l3fwd_acl_send_packets (qconf, pkts_burst,
                                          acl_search.res_ipv6, nb_rx);
                }
            }
        }
    }
