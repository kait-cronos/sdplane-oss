...
static int
em_add_rules (const char *rule_path, struct em_rule **proute_base,
...
em_add_default_v4_rules (void)
{
  ...
  for (i = 0; i < (unsigned int) route_num_v4; i++)
    {
      em_route_base_v4[i].v4_key.ip_dst =
          ipv4_l3fwd_em_route_array[i].key.ip_dst;
      em_route_base_v4[i].v4_key.ip_src =
          ipv4_l3fwd_em_route_array[i].key.ip_src;
      ...
      em_route_base_v4[i].v4_key.proto =
          ipv4_l3fwd_em_route_array[i].key.proto;
  ...
  for (i = 0; i < (unsigned int) route_num_v6; i++)
    {
      ...
      em_route_base_v6[i].v6_key.proto =
          ipv6_l3fwd_em_route_array[i].key.proto;
...
{
  /* ipv4 check */
  if (parm_config.rule_ipv4_name != NULL && parm_config.rule_ipv6_name != NULL)
    {
      /* ipv4 check */
