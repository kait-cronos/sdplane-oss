                       ...
                       uint16_t portid, struct lcore_conf *qconf)
{
  ...
  if (nb_rx > 0)
    {
      rte_prefetch0 (rte_pktmbuf_mtod (pkts_burst[0], struct rte_ether_hdr *) +
                     1);
    }
  ...
  for (i = 1, j = 0; j < nb_rx; i++, j++)
    {
      if (i < nb_rx)
        {
          rte_prefetch0 (
              rte_pktmbuf_mtod (pkts_burst[i], struct rte_ether_hdr *) + 1);
        }
                         ...
                         struct lcore_conf *qconf)
{
  ...
  rte_prefetch0 (rte_pktmbuf_mtod (events[0]->mbuf, struct rte_ether_hdr *) +
                 1);
  ...
  for (i = 1, j = 0; j < nb_rx; i++, j++)
    {
      ...
      if (i < nb_rx)
        {
          rte_prefetch0 (
              rte_pktmbuf_mtod (events[i]->mbuf, struct rte_ether_hdr *) + 1);
        }
      port = mbuf->port;
                               ...
                               struct lcore_conf *qconf, uint16_t *dst_ports)
{
  ...
  for (i = 0, j = 1; i < vec->nb_elem; i++, j++)
    {
      if (j < vec->nb_elem)
        rte_prefetch0 (rte_pktmbuf_mtod (mbufs[j], struct rte_ether_hdr *) +
                       1);
      dst_ports[i] = em_get_dst_port (qconf, mbufs[i],
                                      attr_valid ? vec->port : mbufs[i]->port);
    }
