...
ip netns exec seg6gen-1 bash -lc '
  ...
  echo 1 > /proc/sys/net/ipv6/conf/"$1"/seg6_enabled
  ...
  ip route add 10.0.1.0/24 encap seg6 mode encap segs fc00:1::1,fc00:b::1 dev "$1"
  ip -6 route add fc00:a::1/128 encap seg6local action End.DT4 dev lo
...
ip netns exec seg6gen-2 bash -lc '
  ...
  ip route add 10.0.0.0/24 encap seg6 mode encap segs fc00:1::1,fc00:a::1 dev "$1"
  ip -6 route add fc00:b::1/128 encap seg6local action End.DT4 dev lo
' -- "$PHY_DEV.400"